...
def train_loop(SC,p_weights,params,l,opt_state): # 
        ...
        for e in range(params["EPOCHS"]):
            ...
            (loss,aux),grads = val_grad_vmap(SC,p_weights,params,dots,samples,h_0) # val_grad_vmap ,,v_0
...
plt.suptitle('forward_training_v7, '+title__,fontsize=14)
...
plt.savefig(path_ + 'forward_training_v7_' + dt + '.png')
...
pos_arr = pos_arr[-1,0,:] #0
dots_arr = dots_arr[-1,0,:,:] #0
...
plt.suptitle('forward_training_v7, '+title__,fontsize=10)
for i in range(params["PLOTS"]):
    ...
    for m in range(M):
        ax0.scatter(mod_(VEC_ARR[0,m]-pos_arr[0]),mod_(VEC_ARR[1,m]-pos_arr[1]),color='k',alpha=0.3,marker='+',s=15)
        ax1.scatter(mod_(VEC_ARR[0,m]-pos_arr[0]),mod_(VEC_ARR[1,m]-pos_arr[1]),color='k',alpha=0.3,marker='+',s=15)
    # true
    for ind,n in enumerate(neuron_locs.T):
...
plt.savefig(path_ + 'figs_forward_training_v7_' + dt + '.png')
