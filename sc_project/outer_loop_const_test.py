...
def new_params(params,e):
    ...
    params_["HP_0"] = (INIT_P/(H_P))*rnd.normal(ki[1],(VMAPS,H_P)) ### CHECK/CHANGE
...
def plan(h1vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,weights,params,pos_t_1,dots,vec_t,sel):#(vec_t,r_t_1,hr_t_1,v_t_1,hv_t_1,weights,params):
    ...
    # r_t_true = loss_obj(dots,sel,pos_t,params["SIGMA_R"])###
    ...
    # print('v_plan_loss=',jnp.sum((v_t_-v_t)**2),'r_plan_loss=',(r_t_-r_t)**2) # 'when planning, how much are plans wrong by'
    return r_t,hr_t,v_t # predictions
...
def move(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,pos_t_1,dots,sel,weights,params): # shouldnt be random; should take action dictated by plan...
    # v_t_1_true = neuron_act(params,dots,pos_t_1)### (use true v to debug)
    # print('sanity_check_v_loss=',jnp.sum((v_t_1_true-v_t_1)**2))[=0] # 'when moving, how much are efferent copy signals wrong by
    pos_t = pos_t_1 + vec_t
    ...
    rhat_t_,hr_t = r_predict(v_t,v_t_1,r_t_1,hr_t_1,weights["r"])
    # print('v_move_loss=',jnp.sum((v_t-vhat_t_)**2),'r_move_loss=',(r_t-rhat_t_)**2) # 'when moving, how much are efferent copy signals wrong by'
    # print('r_t_(plan)=',r_t_,'r_t_(move)=',r_t)
    return r_t,hr_t,v_t,pos_t # true
...
def body_fnc(SC,hr_t_1,hv_0,pos_t_1,dots,sel,ind,weights,params,e): # vmap over hr,hv,pos,dots,sel ; 'policy' = sampling new plan/doing action based on reward of prev action
    sample_arr,r_arr_,pos_arr = ([] for _ in range(3))
    r_arr,r_pred_arr,v_arr,v_pred_arr,v_loss_arr,r_loss_arr = ([] for _ in range(6))
    s_ = 0
    ...
    for t in range(params["TRIAL_LENGTH"]):
        if t < params["INIT_STEPS"]:
            ...
            r_t,hr_t,v_t,pos_t = move(h1vec_t,vec_t,v_t_1,hv_0,r_t_1,hr_t_1,pos_t_1,dots,sel,weights,params)
            r_arr.append(r_t)
            v_arr.append(v_t)
            r_t_,_,v_t_ = plan(h1vec_t,v_t_1,hv_0,r_t_1,hr_t_1,weights,params,pos_t_1,dots,vec_t,sel)
            r_pred_arr.append(r_t_)
            v_pred_arr.append(v_t_)
            r_t_1,hr_t_1,v_t_1,pos_t_1 = r_t,hr_t,v_t,pos_t # carry over r/v from true movement
            r_t_1_,v_t_1_ = r_t_,v_t_
            hr_t_1_ = hr_t
        else:
            h1vec_t,vec_t = sample_vec(SC,ind[t])
            r_t,hr_t,v_t,pos_t = move(h1vec_t,vec_t,v_t_1,hv_0,r_t_1,hr_t_1,pos_t_1,dots,sel,weights,params)
            r_arr.append(r_t)
            v_arr.append(v_t)
            r_t_,hr_t_,v_t_ = plan(h1vec_t,v_t_1_,hv_0,r_t_1_,hr_t_1_,weights,params,pos_t_1,dots,vec_t,sel)
            # (planning based on previous planned r_t_1_,v_t_1_,hr_t_1)
            r_pred_arr.append(r_t_)
            v_pred_arr.append(v_t_)
            r_t_1,hr_t_1,v_t_1,pos_t_1 = r_t,hr_t,v_t,pos_t
            r_t_1_,hr_t_1_,v_t_1_ = r_t_,hr_t_,v_t_
        # if t < params["INIT_STEPS"]:
        #     h1vec_t,vec_t = sample_vec(SC,ind[t])
        #     r_t,hr_t,v_t,hv_t,pos_t = move(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,pos_t_1,dots,sel,weights,params,0)
        #     sample_arr.append(1)
        # elif s_ == 0: # plan
        #     h1vec_t,vec_t = sample_vec(SC,ind[t])
        #     r_t,hr_t,v_t,hv_t = plan(h1vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,weights,params,pos_t_1,dots,vec_t,sel)
        #     sample_arr.append(s_)
        #     s_ = sample_action(r_t,ind[t],params)
        #     # print('r_t=',r_t,'s_=',s_)
        # elif s_ == 1: # move
        #     r_t,hr_t,v_t,hv_t,pos_t = move(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,pos_t_1,dots,sel,weights,params,r_t)
        #     sample_arr.append(s_)
        #     s_ = 0
        # r_t_1,hr_t_1,v_t_1,hv_t_1,pos_t_1 = r_t,hr_t,v_t,hv_t,pos_t
        # r_arr_.append(r_t)
        v_loss_arr.append(jnp.log(jnp.mean((v_t-v_t_)**2)))
        r_loss_arr.append(jnp.log((r_t-r_t_)**2))
        pos_arr.append(pos_t)
    return jnp.array(r_arr),jnp.array(r_pred_arr),jnp.array(v_arr),jnp.array(v_pred_arr),jnp.array(pos_arr),jnp.array(v_loss_arr),jnp.array(r_loss_arr) #jnp.array(r_arr),jnp.array(pos_arr),jnp.array(sample_arr)
...
def full_loop(SC,weights,params):
    ...
    r_arr,r_pred_arr = (jnp.zeros((params["TOT_EPOCHS"],params["TRIAL_LENGTH"])) for _ in range(2))
    v_arr,v_pred_arr = (jnp.zeros((params["TOT_EPOCHS"],params["TRIAL_LENGTH"],params["N"])) for _ in range(2))
    ...
    for e in range(E):
        ...
        r_arr_,r_pred_arr_,v_arr_,v_pred_arr_,pos_arr_,v_loss_arr_,r_loss_arr_ = body_fnc(SC,hr_0,hp_0,pos_0,dots,sel,ind,weights,params,e) # r_arr_,pos_arr_,sample_arr_
        r_arr = r_arr.at[e,:].set(r_arr_) # e,t,
        r_pred_arr = r_pred_arr.at[e,:].set(r_pred_arr_) # e,t,
        v_arr = v_arr.at[e,:,:].set(v_arr_) # e,t,v
        v_pred_arr = v_pred_arr.at[e,:,:].set(v_pred_arr_) # e,t,v
        pos_arr = pos_arr.at[e,:,:].set(pos_arr_) # e,t,2
        dots_arr = dots_arr.at[e,:,:].set(dots) # e,3,2
        v_loss_arr = v_loss_arr.at[e,:].set(v_loss_arr_) # e,t
        r_loss_arr = r_loss_arr.at[e,:].set(r_loss_arr_) # e,t
    ...
    return r_arr,r_pred_arr,v_arr,v_pred_arr,pos_arr,dots_arr,v_loss_arr,r_loss_arr #r_arr,pos_arr,sample_arr
...
###
*_,p_weights = load_('/pkl/forward_v8M_08_06-1857.pkl') # /pkl/forward_v8M_08_06-1857.pkl,/sc_project/pkl/forward_v9_225_13_06-1752.pkl,/sc_project/pkl/forward_v9_225_13_06-1752.pkl,/pkl/forward_v8M_08_06-1857.pkl
*_,r_weights = load_('/sc_project/pkl/value_v5_225_13_06-1849.pkl') # /sc_project/pkl/value_v5_225_10_06-0722.pkl,/sc_project/pkl/value_v5_225_13_06-1849.pkl,sc_project/pkl/value_v5_225_10_06-0722.pkl /homes/lrj34/projects/meta_rl_ego_sim/sc_project/pkl/value_v5_225_09_06-1830.pkl
weights['p'] = p_weights
weights['r'] = r_weights
# print("Loaded weights=******************** \n ",weights['p'].keys(),"\n",weights['r'].keys())
###
startTime = datetime.now()
(r_arr,r_pred_arr,v_arr,v_pred_arr,pos_arr,dots_arr,v_loss_arr,r_loss_arr) = full_loop(SC,weights,params) # (r_arr_,pos_arr_,sample_arr_)

# plot training loss
print("Sim time: ",datetime.now()-startTime,"s/epoch=",((datetime.now()-startTime)/TOT_EPOCHS).total_seconds())

# r_arr = r_arr_[-PLOTS:,:] # (3,0,T)
# pos_arr = pos_arr_[-PLOTS:,:,:] # (3,0,T),((-1 if e))
# sample_arr = sample_arr_[-PLOTS:,:]
r_arr = r_arr[-PLOTS:,:] # e,t
r_pred_arr = r_pred_arr[-PLOTS:,:] # e,t
v_arr = v_arr[-PLOTS:,:,:] # e,t,n
v_t_rgb = v_arr.reshape((PLOTS,TRIAL_LENGTH,3,NEURONS**2)) # e,t,n,3
# print('*********************** v_t_rgb=',v_t_rgb[0,0,:,:])
v_pred_arr = v_pred_arr[-PLOTS:,:,:] # e,t,n
v_pred_rgb = jnp.clip(jnp.abs(v_pred_arr.reshape((PLOTS,TRIAL_LENGTH,3,NEURONS**2))),None,1) # e,t,n,3
pos_arr = pos_arr[-PLOTS:,:,:] # e,t,2
dots_arr = dots_arr[-PLOTS:,:,:] # e,3,2
v_loss_arr = v_loss_arr[-PLOTS:,:] # e,t
r_loss_arr = r_loss_arr[-PLOTS:,:] # e,t
...
fig,axis = plt.subplots(3*PLOTS,3,figsize=(10,9*PLOTS)) #9,4x plt.figure(figsize=(12,6))
title__ = f'EPOCHS={TOT_EPOCHS}, VMAPS={VMAPS}, PLAN_ITS={PLAN_ITS}, INIT_STEPS={INIT_STEPS}, TRIAL_LENGTH={TRIAL_LENGTH} \n SIGMA_R={SIGMA_R:.1f}, NEURONS={NEURONS**2}, MODULES={M}, H_P={H_R}, H_R={H_R}'
plt.suptitle('outer_loop_const, '+title__,fontsize=10)
for i in range(params["PLOTS"]):
    ax0 = plt.subplot2grid((3*PLOTS,3),(3*i,0),colspan=3,rowspan=1)
    ...
    for t in range(TRIAL_LENGTH):
        ...
        ax0.scatter(x=t,y=r_pred_arr[i,t],color='k',s=5,marker='o',label='rhat_t')
...
plt.savefig(path_ + 'figs_outer_loop_const_test_true_none_old_' + dt + '.png')
