...
def gaussian_fit(v_t,MODULES,APERTURE):
    x_y_vectors = gen_vectors(MODULES,APERTURE)
    x_data = x_y_vectors[0,:]
    y_data = x_y_vectors[1,:]
    ...
    result_x = jax.scipy.optimize.minimize(curve_obj, init_params_x, args=(x_data, v_t))
    result_y = jax.scipy.optimize.minimize(curve_obj, init_params_y, args=(y_data, v_t))
    ...
    return (mu_opt_x,mu_opt_y),(sigma_opt_x,sigma_opt_y),mean_reward
...
@jit
def r_predict(v_t,pos_t,MODULES,APERTURE,SIGMA_R): # v_t,v_t_1,r_t_1,hr_t_1,r_weights): #PLACEHOLDER
    mean,var,r_fit_t = gaussian_fit(v_t,MODULES,APERTURE)
...
def plan(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,pos_t_1,dot_t_1,dot_vec,val,weights,consts):#,pos_t_1,dots,vec_t,sel(vec_t,r_t_1,hr_t_1,v_t_1,hv_t_1,weights,params):
    INIT_LENGTH,TEST_LENGTH,NONE_PLAN,MODULES,APERTURE,SIGMA_R,SIGMA_A,SIGMA_N,COLORS,THETA_FULL,PRIOR_STAT,PRIOR_PLAN,C_MOVE,C_PLAN = consts
    ...
    r_pred_t,r_fit_t = r_predict(v_full_t,pos_t,MODULES,APERTURE,SIGMA_R)# (using true pos)
...
def move(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,pos_t_1,dot_t_1,dot_vec,val,weights,consts): # sel, shouldnt be random; should take action dictated by plan...
    ...
    v_t = neuron_act_noise(val,THETA_FULL,SIGMA_A,SIGMA_N,dot_t,pos_t)
...
def dynamic_scan(carry_0):
    ...
    def plan_fnc(carry_args):
        ...
        r_t,v_t,hv_t,pos_t,dot_t = plan(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,pos_t_1,dot_t_1,dot_vec,ind,weights,consts)
        r_tot = 0 - C_PLAN ### # r_t
        t += 1 # params["T_PLAN"]
    ...
    def move_fnc(carry_args):
        ...
        *_,C_MOVE,_ = consts
        ...
        r_t,v_t,hv_t,pos_t,dot_t = move(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,pos_t_1,dot_t_1,dot_vec,ind,weights,consts) # ,sel,hr
        r_tot = r_t - C_MOVE ###
        t += 1 # params["T_MOVE"]
...
def body_fnc(SC,hs_0,hv_0,pos_0,dot_0,dot_vec,ind,weights,weights_s,params): # # ,sel,hr PLAN_ITS
    ...
    rp_0 = 0
    rm_0 = 1
    consts = params["INIT_LENGTH"],params["TEST_LENGTH"],params["NONE_PLAN"],params["MODULES"],params["APERTURE"],params["SIGMA_R"],params["SIGMA_A"],params["SIGMA_N"],params["COLORS"],params["THETA_FULL"],params["SIGMA_A"],params["PRIOR_STAT"],params["PRIOR_PLAN"],params["C_MOVE"],params["C_PLAN"]
    v_0 = neuron_act_noise(ind[-1],params["THETA_FULL"],params["SIGMA_A"],params["SIGMA_N"],dot_0,pos_0)
    r_0 = loss_obj(dots,pos_0,params["SIGMA_R"])### ,sel
...
def pg_obj(SC,hs_0,hp_0,pos_0,dot_0,dot_vec,ind,weights,weights_s,params): # ,sel
    ...
    r_init_arr,pos_init_arr,lp_arr,r_arr,rt_arr,val_arr,sample_arr,vec_kl_arr,act_kl_arr,pos_arr = body_fnc_vmap(SC,hs_0,hp_0,pos_0,dot_0,dot_vec,ind,weights,weights_s,params) # ,sel ,params["PLAN_ITS"] [TEST_LENGTH,VMAPS] arrs_(lp,r),aux
...
(r_init_arr,r_arr,rt_arr,sample_arr,pos_init_arr,pos_arr,dots) = other # ,sel
...
save_pkl(weights,'outer_loop_pg_new_v1_')
