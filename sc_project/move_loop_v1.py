...
def kl_loss(policy,consts):
    *_,PRIOR_SIGMA,PRIOR_PLAN,_,_ = consts
    len_ = len(policy[0])
    vec_prior = jax.scipy.stats.norm.pdf(jnp.linspace(-3*PRIOR_SIGMA,3*PRIOR_SIGMA,len_))
...
def new_dot(dot_t,dot_vec,r_t,ALPHA,BETA,DOT_SPEED,key_t_1):
    def gen_dot(args):
        *_,key_t_1 = args
        ...
        dot_0 = rnd.uniform(key_t,shape=(2,),minval=-jnp.pi,maxval=jnp.pi)
        dot_vec = rnd.uniform(subkey_t,shape=(2,),minval=-jnp.pi,maxval=jnp.pi)
...
def plan(vec_t,pos_t_1,dot_t,sel,consts): ### same as move but 0 reward, old pos
    ALPHA,TEST_LENGTH,NONE_PLAN,SIGMA_R,COLORS,THETA,SIGMA_A,PRIOR_STAT,PRIOR_PLAN,C_MOVE,C_PLAN = consts
    pos_t = pos_t_1 + vec_t
...
def dynamic_scan(carry_0):
    ...
    def scan_body(carry_t_1,x):
        ...
        vec_kl,act_kl = kl_loss(policy_t,consts)
    ...
    def plan_fnc(carry_args):
        ...
        *_,consts = theta
        ...
        return (t,args_t,jnp.concatenate([jnp.array([lp_t]),jnp.array([r_tot]),jnp.array([r_t]),val_t,jnp.array([1]),jnp.array([vec_kl]),jnp.array([act_kl]),pos_t_1,dot_t]))
...
def full_loop(SC,weights,params):
    losses,stds,other,weights_s = train_loop(SC,weights,params)
    test_data = test_loop(SC,weights_s,params)
    return losses,stds,other,weights_s,test_data
...
losses,stds,other,weights_s,test_data = full_loop(SC,weights,params) # (loss_arr,actor_loss_arr,critic_loss_arr,kl_loss_arr,vec_kl_arr,act_kl_arr,r_std_arr,l_sem_arr,plan_rate_arr,avg_tot_r_arr,avg_pol_kl_arr,r_init_arr,r_arr,rt_arr,sample_arr,pos_init_arr,pos_arr,dots,sel)
...
(r_init_arr,r_arr,rt_arr,sample_arr,pos_init_arr,pos_arr,dot_arr,sel) = other
