...
def new_params(params, e): # Modify in place
    ...
    params["DOT_0"] = params["POS_0"] + gen_dot(ki[3],VMAPS,N_DOTS,ACTION_SPACE)
...
BATCH = 500
ACTOR_LR = 0.03 # 0.001,0.0008,0.0005,0.001,0.000001,0.0001
...
losses,stds,other,actor_opt_state,critic_opt_state,weights_s = full_loop(SC,weights,params) # (loss_arr,actor_loss_arr,critic_loss_arr,kl_loss_arr,vec_kl_arr,act_kl_arr,r_std_arr,l_sem_arr,plan_rate_arr,avg_tot_r_arr,avg_pol_kl_arr,r_init_arr,r_arr,rt_arr,sample_arr,pos_init_arr,pos_arr,dots,sel)
...
dt = datetime.now().strftime("%d_%m-%H%M%S")
plt.savefig(path_+'outer_loop_pg_new_v1_ppo_'+dt+'.png')
...
# plt.suptitle('outer_loop_pg_new_v1_ppo_, '+title__,fontsize=10)
...
# dt = datetime.now().strftime("%d_%m-%H%M%S")
# path_ = str(Path(__file__).resolve().parents[1]) + '/sc_project/figs/'
# plt.savefig(path_ + 'figs_outer_loop_pg_new_v1_ppo_' + dt + '.png') # ctrl_v7_(v,r,h normal, r_tp changed)

save_pkl_sc((actor_opt_state,critic_opt_state,weights_s),'outer_loop_pg_new_v1_ppo_')
save_test_data(other,'outer_loop_pg_new_v1_ppo_')
