...
def body_fnc(SC,p_weights,params,pos_0,dot_0,dot_vec,h_0,samples):
    ...
    h_t_1 = h_0
    v_t_1 = neuron_act_scalar(params["COLORS"],params["THETA"],params["SIGMA_A"],dot_arr[:,0],pos_arr[:,0])
    ...
    for t in range(params["TOT_STEPS"]):
        ...
        v_t = neuron_act_scalar(params["COLORS"],params["THETA"],params["SIGMA_A"],dot_arr[:,t+1],pos_arr[:,t+1]) # move(pos_arr[:,t+1],dot_arr[:,t+1],params)
...
plt.savefig(path_ + 'forward_new_training_v2_' + dt + '.png')
...
# plt.suptitle('forward_new_training_v2, '+title__,fontsize=10)
...
# dt = datetime.now().strftime("%d_%m-%H%M%S")
# plt.savefig(path_ + 'figs_forward_new_training_v2_' + dt + '.png')

save_pkl((arrs,aux),'forward_new_v2_'+str(M)) # (no rel_vec / loss_v / loss_d)