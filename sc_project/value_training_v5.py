...
def save_pkl(param,str_):  # can't jit (can't pickle jax tracers)
	path_ = str(Path(__file__).resolve().parents[0]) + '/pkl/' # [1],'/scratch/lrj34/'
...
def new_params(params,e):
    ...
    params_["DOTS"] = gen_dots(ki[0],VMAPS,N_DOTS,APERTURE) #key_,rnd.uniform(ki[0],shape=(EPOCHS,VMAPS,N_DOTS,2),minval=-APERTURE,maxval=APERTURE) #jnp.tile(jnp.array([[2,3]]).reshape(1,1,1,2),(EPOCHS,VMAPS,1,1)) #
    params_["SELECT"] = jnp.eye(N_DOTS)[rnd.choice(ki[1],N_DOTS,(VMAPS,))] #rnd.choice(ki[1],jnp.arange(-APERTURE,APERTURE,0.01),(EPOCHS,VMAPS,2))
    params_["SAMPLES"] = rnd.choice(ki[2],M,(VMAPS,T))
    params_["POS_0"] = rnd.choice(ke[3],jnp.arange(-APERTURE,APERTURE,0.01),(VMAPS,2))
    params_["HV_0"] = jnp.zeros((VMAPS,H)) #jnp.sqrt(INIT/(H))*rnd.normal(ki[4],(EPOCHS,VMAPS,H))
    return params_
...
# @partial(jax.jit,static_argnums=())
def body_fnc(SC,v_weights,params,dots,sel,samples,pos_0,h_0,e): # vmap over dots,sel,samples,h_0
...
def value_model_loop(SC,weights,params):
    ...
    E = params["TOT_EPOCHS"]
    loss_arr,loss_std = ([] for _ in range(2)) # s_arr,s_std,
    # v_arr,r_arr,r_hat_arr,pos_arr = ([] for _ in range(4)) # s_arr,s_std,
    optimizer = optax.adamw(learning_rate=params["LR"],weight_decay=params["WD"])
    ...
    for e in range(E):
        ...
        (loss,aux),grads = val_grad_vmap(SC,v_weights,params,dots,sel,samples,pos_0,h_0,e) # val_grad_vmap ,,v_0
        # v_arr_,r_arr_,r_hat_arr_,pos_arr_,_ = aux # no loss_s
        grads_ = jax.tree_util.tree_map(lambda x: jnp.mean(x,axis=0),grads)
        opt_update,opt_state = optimizer.update(grads_,opt_state,v_weights)
        v_weights = optax.apply_updates(v_weights,opt_update)
        ...
        # v_arr,r_arr,r_hat_arr,pos_arr = (jnp.stack(arr,axis=0) for arr in (v_arr,r_arr,r_hat_arr,pos_arr))
        print("e={}",e)
        print("mean loss={}",jnp.mean(loss))
        if e == E-1:
            v_arr,r_arr,r_hat_arr,pos_arr,_ = aux
    loss_arr = jnp.array(loss_arr)
    loss_std = jnp.array(loss_std)
    # v_arr,r_arr,r_hat_arr,pos_arr = (jnp.stack(l_arr,axis=0) for l_arr in (v_arr,r_arr,r_hat_arr,pos_arr))
    # v_arr = jnp.concatenate(v_arr,axis=0)
    # r_arr = jnp.concatenate(r_arr,axis=0)
    # r_hat_arr = jnp.concatenate(r_hat_arr,axis=0)
    # pos_arr = jnp.concatenate(pos_arr,axis=0)
    # cat_arr = lambda arr: jnp.concatenate(arr,axis=0)
    # stack_arr = lambda arr: jnp.stack(arr,axis=0)
    # v_arr,pos_arr = (cat_arr(arr) for arr in [loss_arr,loss_std,v_arr,r_arr,r_hat_arr,pos_arr]) # s_arr,s_std,
    # loss_arr,loss_std,r_arr,r_hat_arr = (stack_arr(arr) for arr in [loss_arr,loss_std,r_arr,r_hat_arr])
    v_weights_trained = v_weights
    return loss_arr,loss_std,v_weights_trained,v_arr,r_arr,r_hat_arr,pos_arr # ,s_arr,s_std

# hyperparams
TOT_EPOCHS = 40000 # 80000
EPOCHS_SWITCH = 2000
EPOCHS = 1
PLOTS = 10
LOOPS = TOT_EPOCHS//EPOCHS
VMAPS = 2200 # 2000,2200,2000,1100,1000,800,500
LR = 0.00004 #0.00008 # 0.000001,0.0001
WD = 0.0001 # 0.0001
# LAMBDA_S = 0.1
INIT = 3 # 5,2
H = 100 # 500,300 
...
*_,v_weights = load_('/sc_project/pkl/value_v5_225_13_06-0128.pkl')
weights['v_weights'] = v_weights
(loss_arr,loss_std,r_weights_trained,v_arr,r_arr,r_hat_arr,pos_arr) = value_model_loop(SC,weights,params) # ,s_arr,s_std
...
plt.suptitle('value_training_v5, '+title__,fontsize=10)
...
plt.savefig(path_ + 'value_training_v5_' + dt + '.png')
...
# v_arr = jnp.linalg.norm(v_arr[-PLOTS:,:,:],axis=-1) # norm? (PLOTS,T),((-1 if e))
r_arr = r_arr[-PLOTS:,:] # (3,0,T),((-1 if e))
r_hat_arr = r_hat_arr[-PLOTS:,:] # (3,0,T),((-1 if e))
pos_arr = jnp.linalg.norm(mod_(pos_arr[-PLOTS:,:,:]),axis=-1) # (3,0,T),((-1 if e))
...
dt = datetime.now().strftime("%d_%m-%H%M%S")
plt.savefig(path_ + 'figs_value_training_v5_' + dt + '.png')

save_pkl((loss_arr,loss_std,v_arr,r_arr,r_hat_arr,r_weights_trained),'value_v5_'+str(M))