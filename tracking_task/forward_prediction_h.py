...
def gen_dots(key,EPOCHS,VMAPS,N_DOTS,APERTURE):
    ...
    dots_0 = rnd.uniform(keys[0],shape=(EPOCHS,VMAPS,1,2),minval=jnp.array([APERTURE/4,APERTURE/4]),maxval=jnp.array([3*APERTURE/4,3*APERTURE/4]))
    dots_1 = rnd.uniform(keys[1],shape=(EPOCHS,VMAPS,1,2),minval=jnp.array([APERTURE/4,-APERTURE/4]),maxval=jnp.array([3*APERTURE/4,-3*APERTURE/4]))
    dots_2 = rnd.uniform(keys[2],shape=(EPOCHS,VMAPS,1,2),minval=jnp.array([-3*APERTURE/4,-APERTURE]),maxval=jnp.array([-APERTURE/4,APERTURE]))
...
def new_params(params,l):
    ...
    M = params["M"]
    H = params["H"]
    ki = rnd.split(rnd.PRNGKey(0),num=10) #l
...
@jit
def RNN_forward(h1vec,v_0,vh_t_1,p_weights): # h_t_1/vh_t_1 THINK... GRU computation of hp_t_1,v_t_1->hp_t,v_t
    # gc.collect()
    W_h1 = p_weights["W_h1"]
    W_v = p_weights["W_v"]
    U_vh = p_weights["U_vh"]
    b_vh = p_weights["b_vh"]
    vh_t = jax.nn.sigmoid(jnp.matmul(W_h1,h1vec) + jnp.matmul(W_v,v_0) + jnp.matmul(U_vh,vh_t_1) + b_vh) # vh_t_1/h_t_1+ jnp.matmul(W_v_z,v_t_1)
    return vh_t #v_t,hp_t

# @partial(jax.jit,static_argnums=())
def plan(h1vec,v_0,h_0,p_weights,params): # h_t_1/vh_t_1 THINK... self,hp_t_1,pos_t_1,v_t_1,r_t_1,weights,params
    # W_r = p_weights["W_r"]
    # h_t_1 = h_0
    vh_t_1 = jnp.concatenate((v_0,h_0))
    for i in range(params["PLAN_ITS"]):
        vh_t_1 = RNN_forward(h1vec,v_0,vh_t_1,p_weights) # h_t_1/vh_t_1
...
# @partial(jax.jit,static_argnums=())
def body_fnc(SC,p_weights,params,dots,samples,pos_0,h_0): # SC
    ...
    for s in samples:
        ...
        h1vec = H1VEC_ARR[:,s]
        v_pred = plan(h1vec,v_t_1,h_0[s,:],p_weights,params) # h_0[s,:]
    ...
    return avg_loss,(v_pred,v_t,pos_t) # pos_t

def train_loop(SC,p_weights,params,l): # ,opt_state SC
        ...
        for e in range(params["EPOCHS"]):
            ...
            h_0 = params["HP_0"][0,0,:,:] # [e,v,s,h]
            val_grad = jax.value_and_grad(body_fnc,argnums=1,allow_int=True,has_aux=True)
            # val_grad_vmap = jax.vmap(val_grad,in_axes=(None,None,None,None,None,None,None),out_axes=(0,0))
...
def forward_model_loop(SC,weights,params):
    ...
    for l in range(L):
        ...
        p_weights,loss_arr_,loss_std_,(v_pred_arr,v_t_arr,pos_arr,dots_arr) = train_loop(SC,p_weights,params,l) #,opt_state,opt_state
...
POS_0 = rnd.choice(ke[3],jnp.linspace(-APERTURE,APERTURE,MODULES),(EPOCHS,VMAPS,2)) #jnp.array([-0.5,0.5]) #rnd.uniform(ke[3],shape=(EPOCHS,VMAPS,2),minval=-APERTURE,maxval=APERTURE) ### FILL IN; rand array [E,V,2]
# V0 = None ### CHANGE TO ACTUAL ACTIVATIONS CORRESPONDING TO POS_0(INIT/(N))*rnd.normal(ke[3],(EPOCHS,VMAPS,N)) # [E,V,N]
HP_0 = (INIT/(H))*rnd.normal(ke[4],(EPOCHS,VMAPS,STEPS,H)) #rnd.normal(ke[4],(EPOCHS,VMAPS,STEPS,H)) # [E,V,H]
...
plt.suptitle('forward_prediction_h, '+title__,fontsize=14)
...
v_pred_rgb = jnp.abs(v_pred_arr[0,0,:].reshape((3,NEURONS**2))) # ((-1 if e))
# print("v_pred_rgb=",v_pred_rgb[0,:],v_pred_rgb[1,:],v_pred_rgb[2,:])
v_t_rgb = v_t_arr[0,0,:].reshape((3,NEURONS**2))
# print("v_t_rgb=",v_t_rgb[0,:],v_t_rgb[1,:],v_t_rgb[2,:])
pos_arr = pos_arr[0,0,:]
dots_arr = dots_arr[0,0,:,:]
...
title__ = f'EPOCHS={TOT_EPOCHS}, VMAPS={VMAPS}, PLAN_ITS={PLAN_ITS}, STEPS = {STEPS}, init={INIT:.2f}, update={LR:.6f}, WD={WD:.5f}, \n SIGMA_A={SIGMA_A:.1f}, NEURONS={NEURONS**2}, MODULES={M}, H={H}'
plt.suptitle('figs_forward_prediction_h, '+title__,fontsize=10)
