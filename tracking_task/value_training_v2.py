...
def value(v_weights,v_t,v_t_1,r_t_1,h_t_1):
    ...
    h_t = jax.nn.sigmoid(jnp.matmul(W_vt,jnp.log(v_t)) + jnp.matmul(W_vt_1,jnp.log(v_t_1)) + W_rt_1*r_t_1 + jnp.matmul(U_h,h_t_1) + b_h)
...
def body_fnc(SC,v_weights,params,dots,sel,samples,pos_0,h_0): # vmap over dots,sel,samples,h_0
    ...
    for i in range(1,params["TRIAL_LENGTH"]):
        if i < params["TRIAL_LENGTH"]:
            v_t_1 = v_arr[i-1,:]
            v_t = v_arr[i,:]
            r_t_1 = r_arr[i-1]
            r_t = r_arr[i]
            r_hat_t,h_t,sel_hat_t = value(v_weights,v_t,v_t_1,r_t_1,h_t)
            ...
            # r_t_1 = r_hat_t
        elif i >= params["TRAIN_LENGTH"]: ###CHANGE
            v_t_1 = v_arr[i-1,:]
            v_t = v_arr[i,:]
...
VMAPS = 1000 # 1100,1000,800,500
LR = 0.001 # 0.000001,0.0001
WD = 0.0001 # 0.0001
LAMBDA_S = 0.1
INIT = -10
H = 400 # 500,300 
...
W_vt0 = 2200*(INIT/(H*N))*rnd.normal(ki[0],(H,N)) # 2200* ,(INIT/(H*N))*
W_vt_10 = 2200*(INIT/(H*N))*rnd.normal(ki[1],(H,N)) # 2200* ,(INIT/(H*N))*
...
plt.suptitle('value_training_2, '+title__,fontsize=14)
plt.subplot(1,2,1)
...
plt.savefig(path_ + 'value_training_v2_' + dt + '.png')
...
plt.suptitle('value_training_v2, '+title__,fontsize=10)
...
dt = datetime.now().strftime("%d_%m-%H%M%S")
plt.savefig(path_ + 'figs_value_training_v2_' + dt + '.png')

# save_pkl((loss_arr,loss_std,p_weights_trained,v_arr,r_arr,r_hat_arr),'value_v2_')