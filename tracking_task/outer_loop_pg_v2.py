...
def v_predict(h1vec_t,v_t_1,hv_t_1,p_weights,params,NONE_PLAN): # self,hp_t_1,pos_t_1,v_t_1,r_t_1,weights,params
...
def plan(h1vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,weights,params):#,pos_t_1,dots,vec_t,sel(vec_t,r_t_1,hr_t_1,v_t_1,hv_t_1,weights,params):
    v_t,hv_t = v_predict(h1vec_t,v_t_1,hv_t_1,weights["p"],params,params["NONE_PLAN"])#(hr_t_1,v_t_1,pos_t_1,weights["v"],params)
...
def move(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,pos_t_1,dots,sel,weights,params): # shouldnt be random; should take action dictated by plan...
    ...
    vhat_t_,_ = v_predict(h1vec_t,v_t_1,hv_t_1,weights["p"],params,params["NONE_PLAN"])
...
def dynamic_scan(carry_0):
    def scan_body(carry_t_1,x):
        ...
        carry_args = (h1vec_t,vec_t,t,args_t,lp_t,theta) # (lp_arr,r_arr,sample_arr) assemble carry with sampled vecs and updated args
        jax.debug.print('\n ************* t={}',t)
        t,args_t,arrs_t = jax.lax.cond(t < params["TEST_LENGTH"],action_fnc,no_op_fnc,(carry_args)) ### CHANGE 300
        jax.debug.print('\n &&&&&&&&&&&&& arrs_return={}',arrs_t)
        return (t,args_t,theta),arrs_t
    def action_fnc(carry_args):
        (h1vec_t,vec_t,t,(*_,rp_t,rm_t,v_t_1,r_t_1),lp_t,theta) = carry_args
        t,args_t,arrs_t = jax.lax.cond(rp_t == 1,plan_fnc,move_fnc,(carry_args))
        return (t,args_t,arrs_t)
    def no_op_fnc(carry_args):
        (h1vec_t,vec_t,t,args_t,lp_t,theta) = carry_args
        # do no_op
        t += 1
        arrs_t = jnp.nan*jnp.empty((4,)) # jnp.ones((4,)) # (lp_t,r_t,sample_t)
        #
        return jax.lax.stop_gradient((t,args_t,arrs_t))
...
def plan_fnc(carry_args):
    ...
    return (t,args_t,jnp.array([lp_t,0,0,t]))
...
def move_fnc(carry_args):
    ...
    return (t,args_t,jnp.array([lp_t,r_t,1,t]))
...
def init_scan(carry_0):
    def scan_body(carry_t_1,x):
        ...
        return (t,args_t,theta),None # arrs_t

    (t,args_init,_),_ = jax.lax.scan(scan_body,carry_0,None,params["INIT_LENGTH"]) # arrs_stack
    # (t,args_init,_) = jax.lax.while_loop(init_cnd,loop_body,(carry_0))
    return t,args_init # ,arrs_stack
...
def body_fnc(SC,hs_t_1,hr_t_1,hv_t_1,pos_t_1,dots,sel,ind,weights,weights_s,params): # PLAN_ITS
    ...
    args_0 = (hs_t_1,hr_t_1,hv_t_1,pos_t_1,dots,sel,ind,rp_t_1,rm_t_1,v_t,r_t)
    theta = (SC,weights,weights_s,params)
    t,args_init = init_scan((t,args_0,theta))
    t,args_final,arrs_stack = dynamic_scan((0,args_init,theta)) # arrs_0
    lp_arr,r_arr,sample_arr,t_arr = (arrs_stack[:,i] for i in range(4))
    return lp_arr,r_arr,t_arr # ,sample_arr

# def qhat_fnc(r_arr_,t_arr_): # [VMAPS,TEST_LENGTH]
#     jax.debug.print('r_arr_.shape={}',r_arr_.shape)
#     jax.debug.print('t_arr_.shape={}',t_arr_.shape)
#     qhat_t_arr = jnp.zeros((r_arr_.shape[0],r_arr_.shape[1]))
#     r_locs_arr = jnp.nan*jnp.empty((r_arr_.shape[0],r_arr_.shape[1]))
#     for i in range(r_arr_.shape[0]): # VECTORISE r_vec_,t_vec_ in zip(r_arr_,t_arr_):
#         # jax.debug.print('\n ()()()()()() i={}',type(i))
#         r_vec_ = r_arr_[i,:]
#         # t_vec_ = t_arr_[i,:]
#         r_vec = r_vec_[~jnp.isnan(r_vec_)]
#         # t_vec = t_vec_[~jnp.isnan(r_vec_)] #/t_vec_ jnp.array([0,
#         jax.debug.print('r_vec={}',r_vec)
#         # jax.debug.print('t_vec={}',t_vec)
#         # t_max = t_vec[-1]
#         l_vec = len(r_vec_)
...
#         for q,t in zip(qhat_arr[::-1],t_vec[::-1]):
#             qhat_t_arr = qhat_t_arr.at[i,:int(t)].set(q)
#         # qhat_padded = jnp.pad(qhat_arr,(0,l_vec-l_red),mode='constant',constant_values=jnp.nan)
...
#     return qhat_t_arr #,r_locs_arr
# qhat_t_arr = qhat_fnc(r_arr_.T,t_arr_.T) # r_locs_arr

def pg_obj(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params):
    def create_reward_array_row(LP,R,T,T_len):
    ...
    lp_arr,r_arr,t_arr = body_fnc_vmap(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params) # ,params["PLAN_ITS"] [TEST_LENGTH,VMAPS] arrs_(lp,r),aux
    r_new,lp_new = jax.vmap(create_reward_array_row, in_axes=(0,0,0,None))(lp_arr.T,r_arr.T,t_arr.T,params["TEST_LENGTH"])
    r_to_go = reward_to_go(r_new)
    b_t = jnp.nanmean(r_to_go,axis=0)
    A_t = r_to_go - b_t
    A_nonan = jnp.where(jnp.isnan(A_t),0,A_t)
    lp_nonan = jnp.where(jnp.isnan(lp_new),0,lp_new)
    obj = -jnp.mean(jnp.nansum(jnp.multiply(lp_nonan,A_nonan),axis=1)) # negative for adam
    return obj #,aux
