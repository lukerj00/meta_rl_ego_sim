...
def dynamic_scan(carry_0):
    ...
    def plan_fnc(carry_args):
        ...
        return (t,args_t,jnp.concatenate([jnp.array([lp_t]),jnp.array([r_tot]),jnp.array([r_t]),jnp.array([1]),jnp.array([pol_ent]),pos_t]))
    def move_fnc(carry_args):
        ...
        *_,C_MOVE,_ = consts
        ...
        return (t,args_t,jnp.concatenate([jnp.array([lp_t]),jnp.array([r_tot]),jnp.array([r_t]),jnp.array([0]),jnp.array([pol_ent]),pos_t]))
...
def init_scan(carry_0):
    def scan_body(carry_t_1,x):
        ...
        return (t,args_t,theta),jnp.concatenate([jnp.array([r_t]),pos_t]) # arrs_t
...
def body_fnc(SC,hs_t_1,hr_t_1,hv_t_1,pos_t_1,dots,sel,ind,weights,weights_s,params): # PLAN_ITS
    ...
    pos_arr = arrs_stack[:,5:]
    # jax.debug.print("pos_arr.shape={}",pos_arr.shape)
    # jax.debug.print("pos_arr={}",pos_arr)
    return r_init_arr,pos_init_arr,lp_arr,r_arr,rt_arr,sample_arr,pol_ent_arr,pos_arr #,t_arr #
...
def pg_obj(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params):
    ...
    r_init_arr,pos_init_arr,lp_arr,r_arr,rt_arr,sample_arr,pol_ent_arr,pos_arr = body_fnc_vmap(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params) # ,params["PLAN_ITS"] [TEST_LENGTH,VMAPS] arrs_(lp,r),aux
    # jax.debug.print("vecs_arrV.shape={}",pos_arr.shape)
    # jax.debug.print("vecs_arrV={}",pos_arr)
    ...
    return obj,(r_std,l_sem,plan_rate,avg_tot_r,avg_pol_ent,r_init_arr.T,r_arr.T,rt_arr.T,sample_arr.T,pos_init_arr.transpose([1,0,2]),pos_arr.transpose([1,0,2])) # ,t_arr.T
...
def full_loop(SC,weights,params):
    ...
    for e in range(E):
        ...
        (loss,(r_std,l_sem,plan_rate,avg_tot_r,avg_pol_ent,r_init_arr,r_arr,rt_arr,sample_arr,pos_init_arr,pos_arr)),grads = loss_grad(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params) # (loss,aux),grads
        # jax.debug.print("vecs_arrT.shape={}",pos_arr.shape)
        # jax.debug.print("vecs_arrT={}",pos_arr)
        ...
        if e>0 and (e % 50) == 0:
            ...
            jax.clear_caches()
    arrs = (loss_arr,r_std_arr,l_sem_arr,plan_rate_arr,avg_tot_r_arr,avg_pol_ent_arr,r_init_arr,r_arr,rt_arr,sample_arr,pos_init_arr,pos_arr,dots,sel) #loss_arr,
...
print('ID_ARR=',ID_ARR)
(loss_arr,r_std_arr,l_sem_arr,plan_rate_arr,avg_tot_r_arr,avg_pol_ent_arr,r_init_arr,r_arr,rt_arr,sample_arr,pos_init_arr,pos_arr,dots,sel),weights = full_loop(SC,weights,params) # r_arr,pos_arr,sample_arr,dots_arr,v_loss_arr,r_loss_arr,r_true_arr
# print(loss_arr)
...
# print('rt_arr.shape=',rt_arr.shape)
