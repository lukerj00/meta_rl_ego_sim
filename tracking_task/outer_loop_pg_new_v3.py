...
def full_loop(SC,weights,params):
    ...
    optax.adam(learning_rate=params["LR"]), #,weight_decay=params["WD"]),
    )
...
dt = datetime.now().strftime("%d_%m-%H%M%S")
plt.savefig(path_+'outer_loop_pg_new_v3_'+dt+'.png')
...
# plt.suptitle('outer_loop_pg_new_v3_, '+title__,fontsize=10)
...
# dt = datetime.now().strftime("%d_%m-%H%M%S")
# path_ = str(Path(__file__).resolve().parents[1]) + '/sc_project/figs/'
# plt.savefig(path_ + 'figs_outer_loop_pg_new_v3_' + dt + '.png') # ctrl_v7_(v,r,h normal, r_tp changed)

save_pkl_sc((opt_state,weights_s),'outer_loop_pg_new_v3_')
save_test_data(other,'outer_loop_pg_new_v3_')