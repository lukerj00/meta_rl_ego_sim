...
def new_params(params,e):
    ...
    params_["POS_0"] = rnd.choice(ki[1],jnp.arange(-APERTURE,APERTURE,0.01),(VMAPS,2))
    params_["SAMPLES"] = rnd.choice(ki[2],M,(VMAPS,STEPS))
    params_["HP_0"] = jnp.sqrt(INIT/(H))*rnd.normal(ki[3],(VMAPS,H))
    return params_
...
def body_fnc(SC,p_weights,params,pos_0,dots,samples,h_0):
    ...
    return avg_loss,(v_pred,v_t,pos_t) # final v_pred,v_t,pos_t
...
#         loss_arr,loss_std,v_pred_arr,v_t_arr,pos_arr = ([] for _ in range(5))
...
#             loss_arr.append(jnp.mean(loss)) # = loss_arr_.at[e].set(jnp.mean(loss))
#             loss_std.append(jnp.std(loss)/jnp.sqrt(params["VMAPS"])) # = loss_std_.at[e].set(jnp.std(loss)/jnp.sqrt(params["VMAPS"]))
#             # v_pred_arr.append(v_pred) # = v_pred_arr.at[e,:,:].set(v_pred) #
#             # v_t_arr.append(v_t) # = v_t_arr.at[e,:,:].set(v_t) ##
#             # pos_arr.append(pos_t) # = pos_arr.at[e,:,:].set(pos_t) ##
#             # jax.debug.print("std={}",jnp.std(loss)/jnp.sqrt(params["VMAPS"]))
#             jax.debug.print("e={}",e)
#             jax.debug.print("mean loss={}",jnp.mean(loss))
#         v_aux = (v_pred,v_t,pos_t,params["DOTS"]) # v_pred_arr,v_t_arr,pos_arr
#         return p_weights,loss_arr,loss_std,v_aux,opt_state
...
def forward_model_loop(SC,weights,params):
    ...
    loss_arr,loss_std = ([] for _ in range(2)) # ,v_pred_arr,v_t_arr,pos_arr
    optimizer = optax.adamw(learning_rate=params["LR"],weight_decay=params["WD"])
    opt_state = optimizer.init(p_weights)
    for e in range(T):
        ...
        jax.debug.print("mean loss={}",jnp.mean(loss))
        if e == E-1:
            v_pred,v_t,pos_t = aux # [VMAPS,N],[VMAPS,N],[VMAPS,2] (final timestep)
    p_weights_trained = p_weights
    return loss_arr,loss_std,p_weights_trained,v_pred,v_t,pos_t,dots
...
(loss_arr,loss_std,p_weights_trained,v_pred,v_t,pos_t,dots) = forward_model_loop(SC,weights,params)
...
plt.suptitle('forward_training_v10, '+title__,fontsize=14)
...
plt.savefig(path_ + 'forward_training_v10_' + dt + '.png')
...
v_pred_rgb = np.clip(jnp.abs(v_pred[-PLOTS:,:].reshape((PLOTS,3,NEURONS**2))),None,1) # ((-1 if e))
# print("v_pred_rgb=",v_pred_rgb[0,:],v_pred_rgb[1,:],v_pred_rgb[2,:])
v_t_rgb = v_t[-PLOTS:,:].reshape((PLOTS,3,NEURONS**2))
# print("v_t_rgb=",v_t_rgb[0,:],v_t_rgb[1,:],v_t_rgb[2,:])
pos_arr = pos_t[-PLOTS:,:] #0
dots_arr = dots[-PLOTS:,:,:] #0
...
plt.suptitle('forward_training_v10, '+title__,fontsize=10)
...
dt = datetime.now().strftime("%d_%m-%H%M%S")
plt.savefig(path_ + 'figs_forward_training_v10_' + dt + '.png')
...
save_pkl((loss_arr,loss_std,v_pred,v_t,pos_t,dots,p_weights_trained),'forward_v10_'+str(M))