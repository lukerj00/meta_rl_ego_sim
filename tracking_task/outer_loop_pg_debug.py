...
def move(h1vec_t,vec_t,v_t_1,hv_t_1,r_t_1,hr_t_1,pos_t_1,dots,sel,weights,consts): # shouldnt be random; should take action dictated by plan...
    INIT_LENGTH,TEST_LENGTH,NONE_PLAN,SIGMA_R,COLORS,THETA,SIGMA_A,C_MOVE,C_PLAN = consts
    pos_t = pos_t_1 + vec_t
...
def dynamic_scan(carry_0):
    ...
    def scan_body(carry_t_1,x):
        ...
        pol_ent = -jnp.dot(policy_t[0],jnp.log(policy_t[0]))
        ...
        carry_args = (h1vec_t,vec_t,t,args_t,lp_t,theta,pol_ent) # (lp_arr,r_arr,sample_arr) assemble carry with sampled vecs and updated args
    ...
    def plan_fnc(carry_args):
        (h1vec_t,vec_t,t,args_t,lp_t,theta,pol_ent) = carry_args
        ...
        return (t,args_t,jnp.concatenate([jnp.array([lp_t]),jnp.array([r_tot]),jnp.array([r_t]),jnp.array([1]),jnp.array([pol_ent]),vec_t]))
    def move_fnc(carry_args):
        ...
        *_,C_MOVE,_ = consts
        ...
        return (t,args_t,jnp.concatenate([jnp.array([lp_t]),jnp.array([r_tot]),jnp.array([r_t]),jnp.array([0]),jnp.array([pol_ent]),vec_t]))
...
def init_scan(carry_0):
    def scan_body(carry_t_1,x):
        ...
        return (t,args_t,theta),jnp.concatenate([jnp.array([r_t]),vec_t]) # arrs_t

    *_,(*_,(INIT_LENGTH,*_)) = carry_0
    (t,args_init,_),rvec_stack = jax.lax.scan(scan_body,carry_0,None,INIT_LENGTH)### # arrs_stack
    return t,args_init,rvec_stack # ,arrs_stack

def body_fnc(SC,hs_t_1,hr_t_1,hv_t_1,pos_t_1,dots,sel,ind,weights,weights_s,params): # PLAN_ITS
    t = 0
    rp_t_1 = 0
    rm_t_1 = 1
    consts = params["INIT_LENGTH"],params["TEST_LENGTH"],params["NONE_PLAN"],params["SIGMA_R"],params["COLORS"],params["THETA"],params["SIGMA_A"],params["C_MOVE"],params["C_PLAN"]
    ...
    t,args_init,rvec_init_arr = init_scan((t,args_0,theta))
    r_init_arr,vecs_init_arr = rvec_init_arr[:,0],rvec_init_arr[:,1:]
    t,args_final,arrs_stack = dynamic_scan((0,args_init,theta)) # arrs_0
    lp_arr,r_arr,rt_arr,sample_arr,pol_ent_arr = (arrs_stack[:,i] for i in range(5)) #,t_arr
    vecs_arr = arrs_stack[:,5:]
    # jax.debug.print("vecs_arr.shape={}",vecs_arr.shape)
    # jax.debug.print("vecs_arr={}",vecs_arr)
    return r_init_arr,vecs_init_arr,lp_arr,r_arr,rt_arr,sample_arr,pol_ent_arr,vecs_arr #,t_arr #

def pg_obj(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params):
    body_fnc_vmap = jax.vmap(body_fnc,in_axes=(None,0,0,0,0,0,0,0,None,None,None),out_axes=(1,1,1,1,1,1,1,1))
    r_init_arr,vecs_init_arr,lp_arr,r_arr,rt_arr,sample_arr,pol_ent_arr,vecs_arr = body_fnc_vmap(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params) # ,params["PLAN_ITS"] [TEST_LENGTH,VMAPS] arrs_(lp,r),aux
    # jax.debug.print("vecs_arrV.shape={}",vecs_arr.shape)
    # jax.debug.print("vecs_arrV={}",vecs_arr)
    ...
    l_sem = jnp.std(obj_arr)/jnp.sqrt(params["VMAPS"])
    avg_tot_r = jnp.mean(r_to_go[:,0])
    r_std = jnp.std(r_to_go[:,0])
    plan_rate = jnp.mean(sample_arr)
    avg_pol_ent = jnp.mean(pol_ent_arr,axis=None)
    return obj,(r_std,l_sem,plan_rate,avg_tot_r,avg_pol_ent,r_init_arr.T,r_arr.T,rt_arr.T,sample_arr.T,vecs_init_arr.transpose([1,0,2]),vecs_arr.transpose([1,0,2])) # ,t_arr.T
...
def full_loop(SC,weights,params):
    ...
    r_std_arr,loss_arr,l_sem_arr,plan_rate_arr,avg_tot_r_arr,avg_pol_ent_arr = (jnp.zeros((params["TOT_EPOCHS"],)) for _ in range(6)) #jnp.zeros((params["TOT_EPOCHS"]))
    weights_s = weights["s"]
    E = params["TOT_EPOCHS"]
    ...
    for e in range(E):
        ...
        (loss,(r_std,l_sem,plan_rate,avg_tot_r,avg_pol_ent,r_init_arr,r_arr,rt_arr,sample_arr,vecs_init_arr,vecs_arr)),grads = loss_grad(SC,hs_0,hr_0,hp_0,pos_0,dots,sel,ind,weights,weights_s,params) # (loss,aux),grads
        # jax.debug.print("vecs_arrT.shape={}",vecs_arr.shape)
        # jax.debug.print("vecs_arrT={}",vecs_arr)
        ...
        r_std_arr = r_std_arr.at[e].set(r_std)
        l_sem_arr = l_sem_arr.at[e].set(l_sem)
        plan_rate_arr = plan_rate_arr.at[e].set(plan_rate)
        avg_tot_r_arr = avg_tot_r_arr.at[e].set(avg_tot_r)
        avg_pol_ent_arr = avg_pol_ent_arr.at[e].set(avg_pol_ent)
        # vecs_init_arr = vecs_init_arr.at[e].set(vecs_init_arr)
        # vecs_arr = vecs_arr.at[e].set(vecs_arr)
        # (from aux)
        ...
        print("e=",e,"r=",avg_tot_r,"r_std=",r_std,"loss=",loss,"l_sem=",l_sem,"ent=",avg_pol_ent,"plan=",plan_rate)
        if e == E-1:
            pass
        ...
        if e>0 and (e % 50) == 0:
            ...
            jax.clear_caches()
    arrs = (loss_arr,r_std_arr,l_sem_arr,plan_rate_arr,avg_tot_r_arr,avg_pol_ent_arr,r_init_arr,r_arr,rt_arr,sample_arr,vecs_init_arr,vecs_arr,dots,sel) #loss_arr,
    return arrs,weights #r_arr,pos_arr,sample_arr,dots_arr
...
VMAPS = 500 ## 500,1100,1000,800,500
LR = 0.0003 # 0.000001,0.0001
WD = 0.0001 # 0.0001
INIT_S = 2
INIT_P = 2 # 0.5
INIT_R = 3 # 5,2
...
(loss_arr,r_std_arr,l_sem_arr,plan_rate_arr,avg_tot_r_arr,avg_pol_ent_arr,r_init_arr,r_arr,rt_arr,sample_arr,vecs_init_arr,vecs_arr,dots,sel),weights = full_loop(SC,weights,params) # r_arr,pos_arr,sample_arr,dots_arr,v_loss_arr,r_loss_arr,r_true_arr
# print(loss_arr)
...
title__ = f'EPOCHS={TOT_EPOCHS}, VMAPS={VMAPS}, TEST_LENGTH={TEST_LENGTH}, INIT_LENGTH={INIT_LENGTH}, update={LR:.6f}, WD={WD:.5f} \n C_MOVE={C_MOVE:.2f}, C_PLAN={C_PLAN:.2f}, SIGMA_R={SIGMA_R:.1f}, SIGMA_A={SIGMA_A:.1f}, NEURONS={NEURONS**2}, MODULES={M}, H_S={H_S}'
plt.suptitle('outer_loop_pg_training_v4, '+title__,fontsize=10)
axes[0,0].errorbar(np.arange(TOT_EPOCHS),avg_tot_r_arr,yerr=r_std_arr/2,color='black',ecolor='lightgray',elinewidth=2,capsize=0)
axes[0,0].set_xlabel('iteration')
axes[0,0].set_ylabel('R_tot')
# ax2 = plt.twinx()
# ax2.plot(np.arange(TOT_EPOCHS),avg_pol_ent_arr,color='blue',linewidth=1)
# ax2.set_ylabel('policy entropy')
# plt.legend(handles=legend_handles,loc='lower left')
axes[0,1].errorbar(np.arange(TOT_EPOCHS),loss_arr,yerr=l_sem_arr/2,color='black',ecolor='lightgray',elinewidth=2,capsize=0)
axes[0,1].set_xlabel('iteration')
axes[0,1].set_ylabel('loss')
axes[1,0].plot(np.arange(TOT_EPOCHS),avg_pol_ent_arr,color='blue',linewidth=1)
axes[1,0].set_xlabel('iteration')
axes[1,0].set_ylabel('entropy')
axes[1,1].plot(np.arange(TOT_EPOCHS),plan_rate_arr,color='blue',linewidth=1)
axes[1,1].set_xlabel('iteration')
axes[1,1].set_ylabel('plan rate')
...
plt.savefig(path_+'outer_loop_pg_training_v4_'+dt+'.png')

# PLOT TESTING DATA
r_init_arr = r_init_arr[-PLOTS:,:] # [PLOTS,TEST_LENGTH]
r_arr = r_arr[-PLOTS:,:] # [PLOTS,TEST_LENGTH]
print('sample_arr.shape=',sample_arr.shape)
sample_arr = sample_arr[-PLOTS:,:] # [PLOTS,TEST_LENGTH]
print('sample_arr.shape=',sample_arr.shape)
print('sample_arr=',sample_arr)
print('vecs_arr.shapeF=',vecs_arr.shape)
vecs_init_arr = vecs_init_arr[-PLOTS:,:,:] # [PLOTS,TEST_LENGTH,2]
vecs_arr = vecs_arr[-PLOTS:,:,:] # [PLOTS,TEST_LENGTH,2]
print('vecs_arr.shapeF=',vecs_arr.shape)
print('vecs_arrF=',vecs_arr)
...
plt.suptitle('outer_loop_pg_v4_test_, '+title__,fontsize=10)
for i in range(params["PLOTS"]):
    ...
    # for t in range(INIT_LENGTH):
    #     ax1.scatter(x=vecs_init_arr[i,t,0],y=vecs_init_arr[i,t,1],color='black',alpha=0.5,s=60,marker='+')
    for t in range(TEST_LENGTH):
        if sample_arr[i,t] == 1:
            ax1.scatter(x=vecs_arr[i,t,0],y=vecs_arr[i,t,1],color='red',alpha=0.4,s=60,marker='o')
        else:
            ax1.scatter(x=vecs_arr[i,t,0],y=vecs_arr[i,t,1],color='black',alpha=0.3,s=60,marker='o')
...
dt = datetime.now().strftime("%d_%m-%H%M%S")
path_ = str(Path(__file__).resolve().parents[1]) + '/sc_project/figs/'
plt.savefig(path_ + 'figs_outer_loop_pg_v4_' + dt + '.png') # ctrl_v7_(v,r,h normal, r_tp changed)

save_pkl(weights,'value_v4_')
