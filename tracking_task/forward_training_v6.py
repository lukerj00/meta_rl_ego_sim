...
# @partial(jax.jit,static_argnums=())
def body_fnc(SC,p_weights,params,dots,samples,h_0):
    ID_ARR,VEC_ARR,H1VEC_ARR = SC
    loss_tot = 0
    pos_t_1 = VEC_ARR[:,0] # VEC_ARR[:,0]
    h_t_1 = h_0
...
def train_loop(SC,p_weights,params,l,opt_state): # ,opt_state
        ...
        for e in range(params["EPOCHS"]):
            ...
            # pos_0 = params["POS_0"][0,0,:] ## (change) same pos_0 across vmaps
            h_0 = params["HP_0"][0,0,:]
            ...
            # jax.debug.print('grads={}',grads["W_r"].shape)
            grads_ = grads #jax.tree_util.tree_map(lambda x: jnp.mean(x,axis=0),grads)##
            # jax.debug.print('grads_={}',grads_["W_r"].shape)
...
def forward_model_loop(SC,weights,params):
    ...
    v_pred_arr,v_t_arr = (jnp.empty((params["TOT_EPOCHS"],params["VMAPS"],params["N"])) for _ in range(2))
    pos_arr = jnp.empty((params["TOT_EPOCHS"],params["VMAPS"],2))
    dots_arr = jnp.empty((params["TOT_EPOCHS"],params["VMAPS"],3,2))
    optimizer = optax.adamw(learning_rate=params["LR"],weight_decay=params["WD"])
    opt_state = optimizer.init(p_weights)
    for l in range(L):
        params = new_params(params,l)
        p_weights,loss_arr_,loss_std_,(v_pred_arr_,v_t_arr_,pos_arr_,dots_arr_),opt_state = train_loop(SC,p_weights,params,l,opt_state) # opt_state,
        loss_arr = loss_arr.at[l*E:(l+1)*E].set(loss_arr_)
        loss_std = loss_std.at[l*E:(l+1)*E].set(loss_std_)
        v_pred_arr = v_pred_arr.at[l*E:(l+1)*E,:,:].set(v_pred_arr_) # (l//(L-1))
        v_t_arr = v_t_arr.at[l*E:(l+1)*E,:,:].set(v_t_arr_) # (l//(L-1))
        pos_arr = pos_arr.at[l*E:(l+1)*E,:,:].set(pos_arr_)
        dots_arr = dots_arr.at[l*E:(l+1)*E,:,:,:].set(dots_arr_)
...
title_ = f'EPOCHS={TOT_EPOCHS}, VMAPS={VMAPS}, PLAN_ITS={PLAN_ITS}, init={INIT:.2f}, update={LR:.6f}, WD={WD:.5f}, \n SIGMA_A={SIGMA_A:.1f}, NEURONS={NEURONS**2}, MODULES={M}, H={H}'
plt.suptitle('forward_training_v6, '+title_,fontsize=14)
...
plt.savefig(path_ + 'forward_training_v6_' + dt + '.png')
...
plt.suptitle('forward_training_v6, '+title__,fontsize=10)
for i in range(params["PLOTS"]):
    ...
    for ind,n in enumerate(neuron_locs.T):
        ax0.scatter(n[0],n[1],color=np.float32(jnp.sqrt(v_pred_rgb[:,ind])),s=np.sum(17*v_pred_rgb[:,ind]),marker='o') ### sum,np.float32((v_pred_rgb[:,ind]))
        print('0:ind=',ind,'n=',n,'v_pred_rgb[:,ind]=',v_pred_rgb[:,ind])
    # ax0.scatter(-pos_arr[0],-pos_arr[1],c='k',marker='+',s=20)
    # both
    for d in range(N_DOTS):
        ax0.scatter(mod_(dots_arr[d,0]-pos_arr[0]),mod_(dots_arr[d,1]-pos_arr[1]),color=(colors_[d,:]),s=52,marker='x') # dots_arr[i,d,0]
        ax1.scatter(mod_(dots_arr[d,0]-pos_arr[0]),mod_(dots_arr[d,1]-pos_arr[1]),color=(colors_[d,:]),s=52,marker='x') # dots_arr[i,d,0]
        print('dashape',dots_arr,'pos_arr=',pos_arr,'mod_=',mod_(dots_arr[d,0]-pos_arr[0]),mod_(dots_arr[d,1]-pos_arr[1]))
    for m in range(M):
        ax0.scatter(mod_(VEC_ARR[0,m]-pos_arr[0]),mod_(VEC_ARR[1,m]-pos_arr[1]),color='k',alpha=0.4,marker='+',s=15)
        ax1.scatter(mod_(VEC_ARR[0,m]-pos_arr[0]),mod_(VEC_ARR[1,m]-pos_arr[1]),color='k',alpha=0.4,marker='+',s=15)
    # true
    for ind,n in enumerate(neuron_locs.T):
        ax1.scatter(n[0],n[1],color=np.float32(jnp.sqrt(v_t_rgb[:,ind])),s=np.sum(17*v_t_rgb[:,ind]),marker='o') ### sum,np.float32((v_t_rgb[:,ind]))
...
plt.savefig(path_ + 'figs_forward_training_v6_' + dt + '.png')
