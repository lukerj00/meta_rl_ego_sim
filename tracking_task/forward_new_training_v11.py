...
def gen_dot_scheduled(key,N_A,N_dot,APERTURE,e,TOT_EPOCHS):
    ...
    ###
    dot_0 = rnd.uniform(keys[0],shape=(N_A,2),minval=-lim_e,maxval=lim_e)#minval=jnp.array([APERTURE/4,APERTURE/4]),maxval=jnp.array([3*APERTURE/4,3*APERTURE/4]))
...
U_z0,_ = jnp.linalg.qr(jnp.sqrt(INIT/(H+H))*rnd.normal(ki[13],(H,H)))
U_r0,_ = jnp.linalg.qr(jnp.sqrt(INIT/(H+H))*rnd.normal(ki[14],(H,H)))
U_hh0,_ = jnp.linalg.qr(jnp.sqrt(INIT/(H+H))*rnd.normal(ki[15],(H,H)))
b_z0 = jnp.sqrt(INIT/(H))*rnd.normal(ki[16],(H,))
b_r0 = jnp.sqrt(INIT/(H))*rnd.normal(ki[17],(H,))
b_hh0 = jnp.sqrt(INIT/(H))*rnd.normal(ki[18],(H,))
W_ap0 = jnp.sqrt(INIT/(N_A+H))*rnd.normal(ki[19],(N_A,H))
W_full0 = jnp.sqrt(INIT/(N_F+H))*rnd.normal(ki[20],(N_F,H))
