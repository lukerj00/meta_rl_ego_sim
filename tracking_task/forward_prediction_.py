...
def new_params(params,e):
    ...
    SAMPLES = rnd.choice(ke[2],M,(EPOCHS,VMAPS,STEPS))
...
def RNN_forward(h1vec,v_0,h_t_1,p_weights): # THINK... GRU computation of hp_t_1,v_t_1->hp_t,v_t
    ...
    U_vh = p_weights["U_vh"]
    b_vh = p_weights["b_vh"]
    vh_t = jax.nn.sigmoid(jnp.matmul(W_h1,h1vec) + jnp.matmul(W_v,v_0) + jnp.matmul(U_vh,h_t_1) + b_vh) # + jnp.matmul(W_v_z,v_t_1)
...
def plan(h1vec,v_0,h_0,p_weights,params): # THINK... self,hp_t_1,pos_t_1,v_t_1,r_t_1,weights,params
    ...
    h_t_1 = RNN_forward(h1vec,v_0,h_0,p_weights) # vh_t_1
    v_t = jnp.matmul(W_r,h_t_1)
...
# @partial(jax.jit,static_argnums=())
def body_fnc(SC,p_weights,params,dots,pos_0,h_0): # SC
    ID_ARR,VEC_ARR,H1VEC_ARR = SC
    loss_tot = 0
    pos_t_1 = params["POS_0"]#VEC_ARR[:,0] #pos_0
    # for s in samples:
    ...
    v_t = move(pos_t_1,dots,params) # pos_t
    ...
    loss = jnp.linalg.norm(v_pred-v_t) # jnp.sum((v_pred-v_t)**2)
    ...
    avg_loss = loss #_tot #/len(samples)
    return loss,(v_pred,v_t,pos_t_1)

def train_loop(SC,p_weights,params): # SC
        loss_arr_,loss_std_ = (jnp.empty((params["EPOCHS"])),jnp.empty((params["EPOCHS"])))
        # optimizer = optax.adam(learning_rate=params["LR"]) #,weight_decay=params["WD"])
        v_pred_arr,v_t_arr = (jnp.empty((params["EPOCHS"],params["N"])) for _ in range(2))
        for e in range(params["EPOCHS"]):
            dots = params["DOTS"] # same dots across vmaps
            h_0 = params["HP_0"]
            pos_0 = params["POS_0"]
            val_grad = jax.value_and_grad(body_fnc,argnums=1,allow_int=True,has_aux=True)
            (loss,aux),grads = val_grad(SC,p_weights,params,dots,pos_0,h_0) # val_grad_vmap ,,v_0
            v_pred,v_t,pos_t = aux
            # grads_ = jax.tree_util.tree_map(lambda x: jnp.mean(x,axis=0),grads)
            p_weights = jax.tree_util.tree_map(lambda x,y: x - params["LR"]*y,p_weights,grads)
            ...
            # jax.debug.print("grads_={}",grads)
            # jax.debug.print("p_weights={}",p_weights)
        v_aux = (v_pred_arr,v_t_arr)
        return p_weights,loss_arr_,loss_std_,v_aux

# @partial(jax.jit,static_argnums=())
def forward_model_loop(SC,weights,params):
    p_weights = weights["p_weights"]
    E = params["EPOCHS"]
    loss_arr = jnp.empty(params["EPOCHS"])
    v_pred_arr,v_t_arr = (jnp.empty((params["PLOTS"],params["N"])) for _ in range(2))
    # optimizer = optax.adam(learning_rate=params["LR"]) #,weight_decay=params["WD"])
    # opt_state = optimizer.init(p_weights)
    p_weights,loss_arr_,loss_std_,(v_pred_arr,v_t_arr) = train_loop(SC,p_weights,params)
    p_weights_trained = p_weights
    return loss_arr_,p_weights_trained,v_pred_arr,v_t_arr
...
NEURONS = 5
N = 3*(NEURONS**2)
SIGMA_A = 1 # 0.5,1,0.3,1,0.5,1,0.1
APERTURE = jnp.pi
...
POS_0 = jnp.array([0.5,0.5]) #rnd.choice(ke[3],jnp.linspace(-APERTURE,APERTURE,MODULES),(EPOCHS,VMAPS,2)) #rnd.uniform(ke[3],shape=(EPOCHS,VMAPS,2),minval=-APERTURE,maxval=APERTURE) ### FILL IN; rand array [E,V,2]
# V0 = None ### CHANGE TO ACTUAL ACTIVATIONS CORRESPONDING TO POS_0(INIT/(N))*rnd.normal(ke[3],(EPOCHS,VMAPS,N)) # [E,V,N]
HP_0 = (INIT/(H))*rnd.normal(ke[4],(H,)) #rnd.normal(ke[4],(EPOCHS,VMAPS,STEPS,H)) # [E,V,H]
...
loss_arr,p_weights_trained,v_pred_arr,v_t_arr = forward_model_loop(SC,weights,params)

# plot training loss
print("Training time: ",datetime.now()-startTime,"s/epoch=",((datetime.now()-startTime)/EPOCHS).total_seconds())
plt.figure(figsize=(12,6))
title__ = f'EPOCHS={EPOCHS}, VMAPS={VMAPS}, PLAN_ITS={PLAN_ITS}, init={INIT:.2f}, update={LR:.6f}, WD={WD:.5f}, \n SIGMA_A={SIGMA_A:.1f}, NEURONS={NEURONS**2}, MODULES={M}, H={H}'
plt.suptitle('forward_prediction_, '+title__,fontsize=14)
plt.plot(jnp.arange(EPOCHS),loss_arr,color='black')
...
v_pred_rgb = v_pred_arr[-1,:].reshape((3,NEURONS**2))
print("v_pred_rgb=",v_pred_rgb[0,:],v_pred_rgb[1,:],v_pred_rgb[2,:])
v_t_rgb = v_t_arr[-1,:].reshape((3,NEURONS**2))
print("v_t_rgb=",v_t_rgb[0,:],v_t_rgb[1,:],v_t_rgb[2,:])
pos_arr = POS_0
dots_arr = DOTS #dots_arr[-1,0,:,:]
...
for i in range(params["PLOTS"]):
    ...
    for ind,n in enumerate(neuron_locs.T):
        print('***********',(v_pred_rgb[:,ind]),v_pred_rgb[:,ind].shape)
        ax0.scatter(n[0],n[1],s=np.sum(10*v_pred_rgb[:,ind]),marker='o',color=np.float32((v_pred_rgb[:,ind]))) #,,sum,np.float32((v_pred_rgb[:,ind]))
        # print('0:ind=',ind,'n=',n,'v_pred_rgb[:,ind]=',v_pred_rgb[:,ind])
    # true
    ...
    for d in range(N_DOTS):
        ax0.scatter(mod_(dots_arr[d,0]-pos_arr[0]),mod_(dots_arr[d,1]-pos_arr[1]),color=(colors_[d,:]),s=150,marker='x') # [d,0],[d,1].. dots_arr[i,d,0]
        ax1.scatter(mod_(dots_arr[d,0]-pos_arr[0]),mod_(dots_arr[d,1]-pos_arr[1]),color=(colors_[d,:]),s=150,marker='x') # dots_arr[i,d,0]
    ...
    for ind,n in enumerate(neuron_locs.T):
        ax1.scatter(n[0],n[1],s=np.sum(10*v_t_rgb[:,ind]),marker='o',color=np.float32((v_t_rgb[:,ind]))) #,,sum,np.float32((v_t_rgb[:,ind]))
        # print('1:ind=',ind,'n=',n,'v_t_rgb[:,ind]=',v_t_rgb[:,ind])
...
plt.savefig(path_ + 'figs_forward_train_' + dt + '.png')
