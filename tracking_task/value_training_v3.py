...
def gen_vr_arr(SC,dots,sel,samples,pos_0,params):
    ...
    pos_arr = pos_arr.at[:,0].set(pos_0) #VEC_ARR[:,rnd.choice(samples[0],jnp.arange(params["M"]),shape=())]) (use odd MODULES)
    ...
    for i,s in enumerate(samples[1:]):
        pos_arr = pos_arr.at[:,i+1].set(pos_arr[:,i] + VEC_ARR[:,rnd.choice(rnd.PRNGKey(s),jnp.arange(params["M"]),shape=())]) # pos_arr[:,i] + choice_, jnp.arange(params["M"]) ,p=p_
...
def value_model_loop(SC,weights,params):
    ...
    for l in range(L):
        ...
        v_weights,loss_arr_,loss_std_,(v_arr_,r_arr_,r_hat_arr_,pos_arr_,s_arr_,s_std_),opt_state = train_loop(SC,v_weights,params,l,opt_state)
...
plt.suptitle('value_training_v3, '+title__,fontsize=14)
