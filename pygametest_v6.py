...
### dot parameters
DOT_SZ = 8 # dot size
P_DOT_NO = 3 # number of positive dots
P_COLORS = [[0, 100, 200], [100, 0, 200], [0,200,100]]
P_REWARD = 100
COLLISION_P = pygame.USEREVENT + 1
N_DOT_NO = 1 # number of negative dots
N_COLORS = [[200,0,0]]
N_REWARD = -100
...
pygame.display.set_caption('sim_v6')
...
def f(dot,arr_j,arr_i,sigma):
    del_j = np.ones([1,N])*dot.x - arr_j[1,:]
    del_i = np.ones([N,1])*dot.y - np.array(arr_i[:,1]).reshape(N,1)
