...
dump_ = open(stdout_ + 'dump30.txt','r').read() # 41,30(s,r,d)
# path_pkl = path_ + "\\pkl\\"
...
for e in EPOCHS_:
    ...
    sel_reg = r'(?<=nsel)(.*)(?=tot)' # buggy end condition.. try /i
...
d=1
v=1
test = dis_arr[(e*(VMAPS*IT)+v*IT):(e*(VMAPS*IT)+(v+1)*IT-1),d]
print('dis_arr',dis_arr.shape,dis_arr,'\n test',test) # e=2,d=1,v=1
...
for e in range(len(EPOCHS_)):
    for v in range(VMAPS):
            for d in range(DOTS):
                ...
                axis[e,v].plot(dis_arr[(e*(VMAPS*IT)+d*IT):(e*(VMAPS*IT)+(d+1)*IT-1),v],color=tuple(colors[d,:]))
