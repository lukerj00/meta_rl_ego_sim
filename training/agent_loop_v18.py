...
def single_step(EHT_t_1,eps):
    ...
    f_t = jax.nn.sigmoid(jnp.matmul(W_f, jnp.concatenate((act_r,act_g,act_b,sel),axis=0)) + jnp.matmul(U_f,h_t_1) + b_f)
    hhat_t = jnp.tanh(jnp.matmul(W_h,jnp.concatenate((act_r,act_g,act_b,sel),axis=0)) + jnp.matmul(U_h,(jnp.multiply(f_t,h_t_1))) + b_h )
