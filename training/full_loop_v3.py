...
# rnd.normal(ki[16],shape=[EPOCHS,IT,2,VMAPS],dtype=jnp.float32
...
def new_theta(theta_,x):
    ...
    theta["ENV"]["DOTS"] = gen_dots(ki[0],N_DOTS,VMAPS,EPOCHS)
    theta["ENV"]["EPS"] = gen_eps(ki[1],EPOCHS,IT,VMAPS)
    theta["ENV"]["SELECT"] = gen_select(ki[2],N_DOTS,EPOCHS,VMAPS)
    return theta
...
SIGMA_RINF = jnp.float32(0.4) # 0.3
...
fig,axis = plt.subplots(2*TESTS,4,figsize=(13,5*TESTS+7))#(4,5)
plt.suptitle(title__,fontsize=14)
for i in range(loop_params["TESTS"]):
    ...
    ax0.set_ylim(-0.2,jnp.pi*jnp.sqrt(2)+0.2) # 2*jnp.pi*jnp.sqrt(2)
