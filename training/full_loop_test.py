...
def abs_pos2(pos_t):
    ...
    pos_t = pos_t.at[:,:,1].set((jnp.pi/2)*jnp.sin(pos_t[:,:,1]))
    return pos_t
...
def full_loop(loop_params,theta_0): # main routine: R_arr, std_arr = full_loop(params)
    ...
    return (vals_test,theta_test) # vals_train_tot,
...
VMAPS = 1000 # 500
TESTS = 5
UPDATE = jnp.float32(0.00002) #0.00005,0.00002,0.0001,0.00005,,0.0001,0.00001,0.0005,0.0001,0.00001,0.00002,0.0001,0.00008
...
(vals_test,theta_test) = full_loop(loop_params,theta_0) #,vals_train,
...
plt.figure()
title__ = f'v7 testing, tot epochs={TOT_EPOCHS}, it={IT}, vmaps={VMAPS}, init={INIT:.2f}, update={UPDATE:.5f}, SIGMA_N={SIGMA_N}, SIGMA_A={SIGMA_A:.1f}, SIGMA_R0={SIGMA_R0:.1f}, SIGMA_RINF={SIGMA_RINF:.1f}, SIGMA_N={SIGMA_N:.1f}, STEP={STEP:.3f} \n WD={WD:.5f}, LAMBDA_D={LAMBDA_D:.4f}, LAMBDA_E={LAMBDA_E:.4f}, LAMBDA_S={LAMBDA_S:.4f}'
