...
def neuron_act(e_t_1,th_j,th_i,SIGMA_A,COLORS,pos):
	...
	E = G.transpose((1,0,2)) - (e_t_1).T # e_t_1-pos
...
@jit
def loss_obj_pos(e_t_1,sel,e,pos,SIGMA_R0,SIGMA_RINF,TAU,LAMBDA_N,LAMBDA_E,LAMBDA_D,LAMBDA_S): # R_t
...
def new_env(e_t_1,v_t,R_t,ALPHA,N_DOTS,VMAPS,EPOCHS,epoch): # create hyperparam for start of dot randomising
    ...
    e_t = jax.lax.cond((jnp.abs(R_t)>ALPHA)&(epoch>=10000),switch_dots,keep_dots,evrnve)#4000
    return e_t

@jit
def abs_dis(e_t): #(for env)
	# e_t_ = (e_t + jnp.pi)%(2*jnp.pi)-jnp.pi
    e_t_ = (e_t + jnp.pi)%(2*jnp.pi)-jnp.pi
    return e_t_ #jnp.sqrt(e_t_[:,0]**2+e_t_[:,1]**2)

@jit
def abs_dist(e_t,pos): #(for agent)
	# e_t_ = (e_t + jnp.pi)%(2*jnp.pi)-jnp.pi
...
def single_step(EHT_t_1,eps):
    ...
    # reward from neurons
    (R_obj,sigma_e) = obj(e_t_1,sel,SIGMA_R0,SIGMA_RINF,TAU,epoch)#loss_obj_pos(e_t_1,sel,epoch,pos_t,SIGMA_R0,SIGMA_RINF,TAU,LAMBDA_N,LAMBDA_E,LAMBDA_D,LAMBDA_S) #obj(e_t_1,sel,SIGMA_R0,SIGMA_RINF,TAU,epoch)
    ...
    R_t = R_obj + LAMBDA_D*R_dot + LAMBDA_S*R_sel ### + LAMBDA_E*R_env LAMBDA_D*R_dot + 
    ...
    EHT_t = (e_t,h_t,theta,pos_t,sel,epoch,dot)
...
def tot_reward(e0,h0,theta,sel,eps,epoch):
    ...
    jax.lax.cond(((epoch%1000==0)|((epoch>=10000)&(epoch%500==0))),true_debug,false_debug,esdr)#4000
    return jnp.sum(R_t)
...
IT = 50 # 50
VMAPS = 500 # 500
UPDATE = jnp.float32(0.0001) # 0.0005
WD = jnp.float32(0.0001) # 0.001
