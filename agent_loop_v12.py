...
@jit
def neuron_act_(e_t_1,th_j,th_i,SIGMA_T,SIGMA_R,N_COLORS,sel_t):
...
@jit
def single_step(EHT_t_1,eps_sel_t):
    # unpack values
    ...
    #SELECT = theta["ENV"]["SELECT"]
    
    # neuron activations
    (sr_t,sg_t,sb_t) = neuron_act_(e_t_1,THETA_J,THETA_I,SIGMA_T,SIGMA_R,N_COLORS,sel_t)
...
def tot_reward(e0,h0,theta,eps_sel):
    EHT_0 = (e0,h0,theta)
    IT = theta["ENV"]["IT"]
    EHT_,R_t = jax.lax.scan(single_step,EHT_0,eps_sel,length=IT)
    return jnp.sum(R_t)
...
def main():
    ...
    APERTURE = jnp.pi/3
    N_COLORS = jnp.float32([[255,0,0],[0,255,0],[0,0,255]]) # must correspond to n_dots
    ...
    #SELECT = jnp.tile(jnp.eye(N_DOTS)[rnd.choice(ki[8],N_DOTS,EPOCHS)],3)
    ...
    for e in range(EPOCHS): #jax.lax.scan again or need loop?
        ...
        eps_sel = 0 ### TO DO: eps_sel = f(eps,SELECT)
        
        # vmap over tot_reward; find avg r_tot, grad
        val_grad_vmap = jax.vmap(jax.value_and_grad(tot_reward,argnums=2,allow_int=True),in_axes=(2,None,None,2),out_axes=(0,0))
        R_tot,grads = val_grad_vmap(e0,h0,theta,eps_sel)
