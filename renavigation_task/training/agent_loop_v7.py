# -*- coding: utf-8 -*-
"""
Created on Mon Dec  5 15:31:58 2022
...
def gen_neurons_j(NEURONS,APERTURE):
    return jnp.reshape(jnp.linspace(-APERTURE,APERTURE,NEURONS,dtype="float32"),[1,NEURONS])
    
#@jit
def gen_neurons_i(NEURONS,APERTURE):
    return jnp.reshape(jnp.linspace(-APERTURE,APERTURE,NEURONS,dtype="float32"),[NEURONS,1])
...
def new_env(e_t_1,v_t): # e_t
    v_t = v_t.reshape([2,]) # careful when adding more dots
    return e_t_1 - v_t
...
def main():
    ...
    for e in range(EPOCHS): # or use jax.lax.scan(single_step,...)
        ...
        e0 = create_dots(theta["ENV_params"]["N_DOTS"],ke[e])
