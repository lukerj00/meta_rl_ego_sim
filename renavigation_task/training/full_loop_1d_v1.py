...
def gen_dots(KEY_DOT,N_DOTS,VMAPS,EPOCHS):
	return rnd.uniform(KEY_DOT,shape=[EPOCHS,N_DOTS,1,VMAPS],minval=-jnp.pi,maxval=jnp.pi,dtype=jnp.float32)
# gen_dots = jit(gen_dots,static_argnums=(1,2,3))

def gen_eps(KEY_EPS,EPOCHS,IT,VMAPS):
    return rnd.normal(KEY_EPS,shape=[EPOCHS,IT,1,VMAPS],dtype=jnp.float32)
...
def neuron_act(COLORS,THETA,SIGMA_A,dots,pos): #e_t_1,th_j,th_i,SIGMA_A,COLORS # COLORS,THETA,SIGMA_A
    ...
    G = jnp.tile(THETA.reshape(N_,1),(1,D_))
    C = (COLORS/255).transpose((1,0))
    E = G - jnp.squeeze((dots-pos)) #.reshape((2,1))
    act = jnp.exp((jnp.cos(E) - 1)/SIGMA_A**2).T #.reshape((D_,N_**2))
    act_r,act_g,act_b = jnp.matmul(C,act) #.reshape((3*N_**2,))
    # act_rgb = jnp.concatenate((act_r,act_g,act_b))
    return act_r,act_g,act_b

@jit
def sigma_fnc(SIGMA_R0,SIGMA_RINF,T,EPOCHS,e,x):
    TAU = jnp.float32((1/(jnp.e))*T)
    e_ = EPOCHS*x+e
...
def loss_env(pos_hat,pos_t): 
    ...
    R_env = -jnp.exp((jnp.cos(theta_e_0 - pos_t) - 1)) # NEGATIVE
    return R_env

@jit
def loss_dot(dot_hat,dot):
    theta_d_0 = jnp.arctan2(dot_hat[1],dot_hat[0]) #x=atan(sinx/cosx)
    R_dot = -jnp.exp((jnp.cos(theta_d_0 - dot) - 1)) # NEGATIVE
    return R_dot
...
def loss_obj(e_t_1,sel,e,pos,SIGMA_R0,SIGMA_RINF,TOT_EPOCHS,EPOCHS,x): # R_t
    ...
    e_rel = e_t_1-pos
    obj = -jnp.exp((jnp.cos(e_rel) - 1)/sigma_e**2) ### standard loss (-) (1/(sigma_e*jnp.sqrt(2*jnp.pi)))*
    R_obj = jnp.dot(jnp.squeeze(obj),sel)
    p_ = (1/(sigma_e*jnp.sqrt(2*jnp.pi)))*(-R_obj) - (1/(5*sigma_e*jnp.sqrt(2*jnp.pi)))*((-R_obj)**2) #(check normalised correctly... bessel fncs?)
    sample_ = rnd.bernoulli(rnd.PRNGKey(jnp.int32(jnp.floor(10000*(pos[0]-R_obj)))),p=p_/5)
...
def geod_dist(dots,pos):### calculate geodesic
    ...
    th1 = jnp.minimum(jnp.abs(pos_-dots),2*jnp.pi-jnp.abs(pos_-dots))
    return jnp.sqrt(th1**2)
...
def switch_dots(evrnve):###change to distance-based
    ...
    key1 = rnd.PRNGKey(jnp.int32(jnp.floor(1000*v_t)))
    key2 = rnd.PRNGKey(jnp.int32(jnp.floor(1000*(v_t+1))))
    e_t_1 = e_t_1.at[:].set(rnd.uniform(key1,shape=[3,],minval=-jnp.pi,maxval=jnp.pi,dtype=jnp.float32))
    pos_t = pos_t.at[:].set(rnd.uniform(key2,shape=[1,],minval=-jnp.pi,maxval=jnp.pi,dtype=jnp.float32))
    return e_t_1,pos_t    

def switch_agent(pos_t):
    pos_t = rnd.uniform(rnd.PRNGKey(jnp.int32(jnp.floor(1000*jnp.squeeze(pos_t)))),shape=[1,],minval=-jnp.pi,maxval=jnp.pi,dtype=jnp.float32)
    return pos_t
...
def single_step(EHT_t_1,eps):
    ...
    # neuron activations
    (act_r,act_g,act_b) = neuron_act(COLORS,THETA,SIGMA_A,e_t_1,pos_t)
    
    # reward from neurons
    R_rgb,sample_ = loss_obj(e_t_1,sel,epoch,pos_t,SIGMA_R0,SIGMA_RINF,TOT_EPOCHS,EPOCHS,x)
    R_r,R_b,R_g = R_rgb
    R_temp = jnp.dot(jnp.squeeze(R_rgb),sel)
    ...
    # new env    
    dot = jnp.dot(sel,jnp.squeeze(e_t_1))
    ...
    # assemble output
    EHT_t = (e_t_1,h_t,theta,pos_t,sel,epoch,dot.reshape((1,)),R_obj,jnp.squeeze(R_env),jnp.squeeze(R_dot),R_sel,x)#R_tot
...
def tot_reward(e0,h0,theta,sel,eps,epoch,x):
    pos_t=jnp.array([0],dtype=jnp.float32)#jnp.float64
    dot=jnp.array([0],dtype=jnp.float32)#jnp.float64
    LAMBDA_E = theta["ENV"]["LAMBDA_E"]
    LAMBDA_D = theta["ENV"]["LAMBDA_D"]
    LAMBDA_S = theta["ENV"]["LAMBDA_S"]
...
def train_body(e,LTORS): # (body_fnc) returns theta etc after each trial
    ...
    jax.debug.print('e={}',x*loop_params["EPOCHS"]+e)
    jax.debug.print('loss={}',R_tot[e])
...
def test_loop(loop_params,theta_test):
    pos_arr = jnp.empty([loop_params["TESTS"],loop_params["VMAPS"],loop_params["IT"]+1,1])
    switch_arr,R_test,R_r,R_g,R_b = (jnp.empty([loop_params["TESTS"],loop_params["VMAPS"],loop_params["IT"]+1]) for _ in range(5))
    R_t = R_test #,R_r,R_g,R_b)
    pos_arr = pos_arr.at[:,:,0,:].set(jnp.float32([0]))
...
SIGMA_R0 = jnp.float32(0.1) # 0.5,0.7,1,0.5,,0.8,0.5,0.8,0.5
SIGMA_RINF = jnp.float32(0.1) # 0.15,0.3,0.6,1.8,0.1,,0.3
SIGMA_N0 = jnp.float32(0.5) # 2,1.6,,1,2,0.3, 1.8,1.6
SIGMA_NINF = jnp.float32(0.5) # 1,0.8,,0.3
LAMBDA_N = jnp.float32(0.0001)
LAMBDA_E = jnp.float32(0.01) ### 0.12,0.10,0.008,,0.04,0.1,0.05,0.01,0.1
LAMBDA_D = jnp.float32(0.01) ### 0.06,,0.08,0.06,0.07,0.03,0.04,0.01,0.001 
LAMBDA_S = jnp.float32(0.005) ### 0.03,0.05,,0.015,0.0024,0.0012,0.001,0.0001,0.00001
ALPHA = jnp.float32(0.8) # 0.1,0.7,0.99
STEP = jnp.float32(0.03) # 0.02,0.1, play around with! 0.05,,0.002,0.005
APERTURE = jnp.pi #(fully observed)pi/3
...
# GRU parameters
N = NEURONS # **2
G = 100 #100,80, (same as loaded weights)
INIT = jnp.float32(2) # 15-300..,0.3,0.5,0.1,0.2,0.3,,0.5,0.1
...
VMAPS = 1500 # 500
TESTS = 5 # 20
UPDATE = jnp.float32(0.00001) #0.000004, 0.00003,0.00005,0.00003,0.00001,0.00002,0.0001,0.00005,,0.0001,0.00001,0.0005,0.0001,0.00001,0.00002,0.0001,0.00008
WD = jnp.float32(0.0001) # 0.0005,0.001,0.0001,0.00005,0.00001
...
Wr_z0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[1],(G,N),dtype=jnp.float32)
Wg_z0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[1],(G,N),dtype=jnp.float32)
Wb_z0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[1],(G,N),dtype=jnp.float32)
U_z0 = jnp.sqrt(INIT/(G+G))*rnd.normal(ki[2],(G,G),dtype=jnp.float32)
b_z0 = jnp.sqrt(INIT/G)*rnd.normal(ki[3],(G,),dtype=jnp.float32)
Wr_r0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[4],(G,N),dtype=jnp.float32)
Wg_r0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[4],(G,N),dtype=jnp.float32)
Wb_r0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[4],(G,N),dtype=jnp.float32)
U_r0 = jnp.sqrt(INIT/(G+G))*rnd.normal(ki[5],(G,G),dtype=jnp.float32)
b_r0 = jnp.sqrt(INIT/G)*rnd.normal(ki[6],(G,),dtype=jnp.float32)
Wr_h0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[7],(G,N),dtype=jnp.float32)
Wg_h0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[7],(G,N),dtype=jnp.float32)
Wb_h0 = jnp.sqrt(INIT/(G+N))*rnd.normal(ki[7],(G,N),dtype=jnp.float32)
U_h0 = jnp.sqrt(INIT/(G+G))*rnd.normal(ki[8],(G,G),dtype=jnp.float32)
b_h0 = jnp.sqrt(INIT/G)*rnd.normal(ki[9],(G,),dtype=jnp.float32)
W_r_z0 = jnp.sqrt(INIT/G)*rnd.normal(ki[10],(G,),dtype=jnp.float32)
W_r_f0 = jnp.sqrt(INIT/G)*rnd.normal(ki[11],(G,),dtype=jnp.float32)
W_r_hh0 = jnp.sqrt(INIT/G)*rnd.normal(ki[12],(G,),dtype=jnp.float32)
C0 = jnp.sqrt(INIT/(G))*rnd.normal(ki[13],(G,),dtype=jnp.float32)
E0 = jnp.sqrt(INIT/(2+G))*rnd.normal(ki[14],(2,G),dtype=jnp.float32)
D0 = jnp.sqrt(INIT/(2+G))*rnd.normal(ki[15],(2,G),dtype=jnp.float32)
S0 = jnp.sqrt(INIT/(N_DOTS+G))*rnd.normal(ki[16],(N_DOTS,G),dtype=jnp.float32) # (check produce logits)
...
THETA = gen_neurons(NEURONS,APERTURE)
...
# file_ = 'v8_theta_trained_02_05-0949.pkl' # 'v8_theta_trained_02_05-0953.pkl','v8_theta_trained_01_05-2013.pkl','v8_theta_trained_30_04-1707.pkl','v8_theta_test_trained_26_04-1810.pkl','1D_theta_test_trained_24_04-1433.pkl'
# theta_0_ = load_(file_) #(load_scratch)
# theta_0["GRU"] = theta_0_ #theta_0_ for newer versions
...
plt.suptitle('1D training ' + title__,fontsize=14)
plt.subplot(2,3,1)
...
dt = datetime.now().strftime("%d_%m-%H%M")
plt.savefig(path_ + '1D_train_' + dt + '_' + '.png')

#analyse testing...
pos_arr,switch_arr,R_test = vals_test # ,[TESTS,VMAPS,IT],, dis_arr, R_obj_t,R_env_t,R_dot_t,R_sel_t,dis_t,pos_t = vals_test
...
# plot testing
plt.figure()
title__ = f'1D testing, tot epochs={TOT_EPOCHS}, it={IT}, vmaps={VMAPS}, init={INIT:.2f}, update={UPDATE:.5f}, SIGMA_A={SIGMA_A:.1f}, SIGMA_R0={SIGMA_R0:.1f}, SIGMA_RINF={SIGMA_RINF:.1f}, \n SIGMA_N={SIGMA_N:.1f}, STEP={STEP:.3f} WD={WD:.5f}, LAMBDA_D={LAMBDA_D:.4f}, LAMBDA_E={LAMBDA_E:.4f}, LAMBDA_S={LAMBDA_S:.4f}, NEURONS={NEURONS}, p=R_norm/{7}'
fig,axis = plt.subplots(2*TESTS,1,figsize=(12,4*TESTS+2))#(4,5)
plt.suptitle('1D testing '+title__+f', p=R_norm/{5}',fontsize=14)
for i in range(loop_params["TESTS"]):
    k = rnd.randint(ki[i],(),0,loop_params["VMAPS"]) # rnd.choice(ki[18+j],loop_params["VMAPS"],replace=False)
    ax0 = plt.subplot2grid((2*TESTS,1),(2*i,0),colspan=1)
    ax0.set_ylabel(r'$angle$',fontsize=16)
    ax0.set_xlabel(r'time',fontsize=16)
    pos_ = pos_arr[i,k,:,:] # [TESTS,VMAPS,IT,1]
    sel_ = theta_test["ENV"]["SELECT"][i,k,:] # [EPOCHS,VMAPS,3]
    dots_ = theta_test["ENV"]["DOTS"][i,:,:,k] # [EPOCHS,3,1,VMAPS]
    ax0.plot(jnp.arange(IT+1),jnp.squeeze(pos_),color=colormap)
    ax0.axhline(y=0, color='k', linestyle='--',linewidth=0.5)
    for j,dot in dots_:
        ax0.axhline(y=dot, color=colors_[j,:], linestyle='--',linewidth=0.5)
    ax1 = plt.subplot2grid((2*TESTS,1),(2*i+1,0),colspan=1)
    ax1.set_ylabel(r'$reward$',fontsize=16)
    ax1.set_xlabel(r'time',fontsize=16)
    ax1.plot(jnp.arange(IT+1),jnp.squeeze(R_test[i,k,:]),color='k')
    plt.tight_layout()
    plt.subplots_adjust(top=0.9)

plt.savefig(path_ + '1D_test_' + dt + '_' + '.png')
...
# plt.savefig(path_ + 'm_test_' + dt + '_' + file_ + '.png')

# theta_test["ENV"].pop("EPS")
# theta_test["ENV"].pop("H0")
# save_pkl((vals_train,vals_test,theta_test),'1D_all_' + file_[:-4])
# save_pkl(theta_test,'1D_theta_test_trained')