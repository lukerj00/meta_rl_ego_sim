...
def body_fnc(e,UTORR): # returns theta
    ...
    optimizer = optax.adamw(learning_rate=UPDATE,weight_decay=0.0001) # theta["ENV"]["OPT"] # put in GRU?
    e0 = theta["ENV"]["DOTS"][e,:,:,:]
    h0 = theta["GRU"]["h0"]
...
title__ = f'v22c, epochs={EPOCHS}, it={IT}, vmaps={VMAPS}, update={UPDATE:.4f}, SIGMA_A={SIGMA_A:.1f}, SIGMA_RINF={SIGMA_RINF:.1f}, SIGMA_N={SIGMA_N:.1f} \n colors={jnp.array_str(COLORS[0][:]) + jnp.array_str(COLORS[1][:]) + jnp.array_str(COLORS[2][:])}' #  + jnp.array_str(COLORS[3][:]) + jnp.array_str(COLORS[4][:])}'
