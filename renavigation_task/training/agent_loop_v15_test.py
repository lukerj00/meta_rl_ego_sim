...
def tot_reward(e0,h0,theta,sel,eps,epoch): # can't jit (can't save jax tracers)
    ...
    if (epoch==0)|(epoch%50==0)|(epoch==(theta["ENV"]["EPOCHS"]-1)):
        ###
        ###
        csv_write(dis,'csv_3_new_TEST.csv',1) ### dt?
        ###
        ###
    return jnp.sum(R_t)

# main routine
def main(): # can't jit if want to retrieve values
    # ENV parameters
    ...
    ### save_params(theta,'pkl_theta_.pkl') # no need to examine theta yet
    ###
    ###
    # save_params(COLORS,'pkl_colors_3_.pkl') # '+dt+' # can't save tracers
    # save_params(theta["ENV"]["SELECT"],'pkl_select_3_.pkl') # '+dt+' # can't save tracers
    ...
    ###
    ###
    plt.savefig(path_ + 'fig_TEST' + '.png') # dt?
    plt.show()
