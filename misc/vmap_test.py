...
# d2 = test_dict["a"]
# # print(d2)
# td_2 = {"a": {"b":3,"c":4}, 'd':{'e':3,'f':4}}
# print(td_2)
# td_2['a'] = d2
# print(td_2)
EPOCHS = 10
VMAPS = 2
# SAMPLES = rnd.split(rnd.PRNGKey(0),num=EPOCHS/2*VMAPS*3).reshape(EPOCHS,VMAPS,3)
# print(SAMPLES.shape,type(SAMPLES),SAMPLES[0])
MODULES = 5
M = MODULES**2
ke = rnd.split(rnd.PRNGKey(0),num=4)

ID_ARR = rnd.shuffle(ke[2],jnp.arange(0,MODULES))
# VEC_ARR = gen_vectors(MODULES,APERTURE)
D = jnp.diag(jnp.ones(MODULES))
D2 = jnp.diag(jnp.ones(MODULES))[:,ID_ARR]
H1VEC_ARR = rnd.shuffle(ke[3],D,axis=1)
ID = rnd.choice(ke[2],M,(EPOCHS,VMAPS))
