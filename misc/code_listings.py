...
# theta_0_ = load_('v8_theta_test_trained_24_04-1433.pkl')
...
# get better final loop then simplify to be original loop...
#&
##########################################################################################
...
def full_loop(params, init_weights_p ,weights_v):
    """ full PPO training loop: outer loop over epochs, inner loop over mini batches updates.
    ...
    for epoch in range(params["EPOCHS"]): # outer optimisation loop
        ...
        for b, batch in enumerate(range(0, params["VMAPS"], params["BATCH_SIZE"])): # inner mini batch update loop
            ...
            for _ in range(params["CRITIC_UPDATES"]): # multiple critic updates for each mini-batch of old/new trajectories
                ...
                isolated_batch_loss = lambda new_weights_p: get_batch_losses(old_trajectories_batch, scan_params_batch, params, new_weights_p, weights_v) # get_losses as a univariate function of new_weights_p
            ...
            sem_arrays = {key: sem_arrays[key].at[ind].set(sems[key]) for key in sem_arrays.keys()} # populate standard error arrays
...
def get_batch_losses(old_trajectories_batch, scan_params_batch, params, new_weights_p, weights_v):
    ...
    Args:   old_trajectories_batch: batch of old trajectories, pytree of arrays [B,T].
            scan_params_batch: parameters to batch over, tuple of arrays [B,T].
    ...
    Returns:    (loss_actor, loss_critic): actor and critic losses, tuple of scalars.
                (losses, sems): auxiliary debugging information (losses, sems), tuple of pytrees.
    """
    ...
    (log_probs_new, rewards, values, kl_vectors, kl_decisions) = new_trajectories
    
    (log_probs_old, mask_array, _, decisions) = old_trajectories_batch
    ...
    loss_actor = - loss_ppo - params["LAMBDA_KL_VECTORS"] * loss_kl_vectors - params["LAMBDA_KL_DECISIONS"] * loss_kl_decisions # consider KL losses as part of actor loss; each loss is negated, as optimiser minimises loss
...
def get_old_trajectory(scan_params_i, params, old_weights_p, weights_v):
    ...
    Args:   scan_params_i: i'th scan parameters, pytree of arrays [T,].
            params: training hyperparameters and environment parameters, pytree of scalars.
            old_weights_p: old policy network weights, pytree.
            weights_v: (static) pre-trained visual model network weights, pytree.
...
get_old_trajectories_vmap = jax.vmap(get_old_trajectory, in_axes=(0, None, None, None), out_axes=0) # scan_params input is tuple of arrays [N,T]
...
def get_new_trajectory(old_trajectory_j, scan_params_j, params, new_weights_p, weights_v):
    ...
    Args:   old_trajectory_j: j'th old trajectory, pytree of arrays [T,].
            scan_params_j: j'th scan parameters from old trajectory, tuple of arrays [T,].
            params: training hyperparameters and environment parameters, pytree of scalars.
            new_weights_p: new policy network weights, pytree of arrays.
            weights_v: (static) pre-trained visual model network weights, pytree of arrays.
...
get_new_trajectories_vmap = jax.vmap(get_new_trajectory, in_axes=(0, 0, None, None, None), out_axes=0) # old_trajectories_batch and scan_params_batch inputs are tuples of arrays [B,T]
...
# @jit
# def get_advantages(rewards, values_new, gamma):
#     T = rewards.shape[1]
#     advantages = jnp.zeros_like(rewards)
    
#     for t in range(T - 1):
#         delta = rewards[:, t] + gamma * values_new[:, t + 1] - values_new[:, t]
#         advantages = advantages.at[:, t].set(delta)
#     advantages = advantages.at[:, -1].set(rewards[:, -1] - values_new[:, -1])
    
#     return advantages

# jax.debug.print("lp_ratio={}", jnp.sum((log_probs_new / log_probs_old) * mask_array) / jnp.sum(mask_array)) # ensure the log prob ratio is correctly clipped between (1-eps,1+eps)

# old_trajectories = (log_probs_old, mask_array, vectors, decisions), old_debug = (rewards_old_, values_old_, kl_vectors_old_, kl_decisions_old_)

def new_trajectory_step(carry_t, scan_args_t):
    """ obtain new trajectory timestep by considering old trajectory timestep.

    Args:   carry_t: current carry, tuple of multiple types.
            scan_args_t: current input, tuple of scalars.

    Returns:    carry_t1: next carry, tuple of multiple types.
                (new_trajectory_t, new_debug_t): next output, tuple of scalars.
    """
    return carry_t1, (new_trajectory_t1, new_debug_t1)

def old_trajectory_step(carry_t, _):
    """ obtain old trajectory timestep.

    Args:   carry_t: current carry, tuple of multiple types.
            _: current input, None.

    Returns:    carry_t1: next carry, tuple of multiple types.
                (old_trajectory_t1, old_debug_t1): next output, tuple of scalars.
    """
    return carry_t1, (old_trajectory_t1, old_debug_t1)

@jit
def compute_ppo_loss(advantages, log_probs_old, log_probs_new, mask_array, epsilon):
    """ compute PPO loss using the clipped surrogate objective.

    Args:   advantages: generalised advantage estimates, array [B,T].
            log_probs_old: log probabilities of actions + decisions under old policy, array [B,T].
            log_probs_new: log probabilities of actions + decisions under new policy, array [B,T].
            mask_array: binary mask to mask out refactory periods, array [B,T].
            epsilon: clipping parameter for PPO loss, scalar.

    Returns:    loss_ppo: PPO loss, scalar.
                sem_ppo: standard error of the mean for the PPO loss, scalar.
    """
    return loss_ppo, sem_ppo

@jit
def compute_ppo_loss(advantages, log_probs_old, log_probs_new, mask_array, epsilon):
    """ compute PPO loss using the clipped surrogate objective.

    Args:   advantages: advantage estimates, array [B,T].
            log_probs_old: log probabilities of actions + decisions under the old policy, array [B,T].
            log_probs_new: log probabilities of actions + decisions under the new policy, array [B,T].
            mask_array: binary mask to mask out refactory periods, array [B,T].
            epsilon: clipping parameter for PPO loss, scalar.
    ...
    clipped_ratio = jnp.minimum(ratio, jnp.clip(ratio, 1 + epsilon, 1 - epsilon)) # clip probability ratios

    loss_ppo = -jnp.sum(jnp.minimum(advantages * ratio, advantages * clipped_ratio) * mask_array) / jnp.sum(mask_array) # clipped surrogate objective
    sem_ppo = jnp.std(loss_ppo * mask_array) / jnp.sum(mask_array) ** 0.5

    return loss_ppo, sem_ppo

def compute_gae(rewards, values, gamma, lambda_):
    """ compute generalized advantage estimate (GAE) and returns.

    Args:   rewards: trajectory rewards, array [B,T].
            values: trajectory estimated values, array [B,T].
            gamma: discount factor for future rewards, scalar.
            lambda_: decay parameter for GAE, scalar.

    Returns:    advantages: GAE for each timestep, array [B,T].
    """
    N, T = rewards.shape
    advantages = jnp.zeros_like(rewards) # initialise advantage array

    last_timestep = T - 1
    delta = rewards[:, last_timestep] - values[:, last_timestep] # GAE for final timestep
    advantages = advantages.at[:, last_timestep].set(delta)

    gae = delta
    for t in reversed(range(T - 1)): # GAE for all other timesteps
...
def new_params(params,epoch):
    """ update parameters at epoch to generate new environments.

    Args:   params: current training parameters (hyperparameters and environment information), pytree.
            epoch: current epoch, scalar.

    Returns:    params: new training parameters, pytree.
    """
    return params
...
def new_params(params,epoch):
    """ update environment parameters at epoch to generate new trajectories.

    Args:   params: current training parameters (hyperparameters and environment parameters), pytree.
            epoch: current epoch, scalar.
...
def get_trajectory(h0, dots, select, epsilon_n, weights, params):
    ...
    Args:   h0: initial hidden state for the agent network, array [H,].
            dots: locations of the 3 RGB dots, array [3,2].
            select: one-hot vector denoting the rewarded dot in that trajectory, array [3,].
...
def single_step(args, epsilon):
    ...
    rdot_hat_t1 = weights["D"] @ h_t1 # predict rewarded dot location
    sel_hat_t1 = weights["S"] @ h_t1 # predict rewarded dot index
