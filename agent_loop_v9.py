...
def main():
    ...
    APERTURE = jnp.pi/3
    N_COLORS = jnp.float32([[255,0,0],[0,255,0]])
    ...
    for e in range(EPOCHS): # or use jax.lax.scan(tot_reward,...)
        ...
        R_tot, grads = jax.value_and_grad(tot_reward,argnums=2,allow_int=True)(e0,h0,theta,eps,IT)
        grads_ = grads["GRU"]
