...
# notes: 
# make sure dots generated within segment
# add penalty for interacting
# create Dot class which also has attributes colour, value
# ideas for disappear: x- -> x (expensive) ; r -> (thought, best) ; empirical (could work...)
...
FPS = 60
VEL = 3
# SEG_ANGLE = GRID_RAD/SPHERE_RAD
# -90 -> -1 , 90 -> 1
# -15 -> -x* -> -grid_rad, 15 -> x* -> grid_rad
# normal: x_ = x* + rad, y_ = (-y* + r)
#map from x to th, apply sin, map to x'
# x* = sin((dot.x-grid_rad)*(seg_angle/grid_rad))
# x = grid_rad + np.sin(((dot.x+dot.width/2)-grid_rad)*(seg_angle/grid_rad))*(grid_rad/sin(seg_angle))

# y* = sin((grid_rad-dot.y)*(seg_angle/grid_rad))
# y = grid_rad - np.sin((grid_rad-(dot.y+dot.height/2))*(seg_angle/grid_rad))*grid_rad/sin(seg_angle)
# x - x_normp
...
pygame.display.set_caption('sim_v3')
...
def draw_window(agent, dots_p, dots_n, score, iteration):
    ...
    for dot in dots_p.values():
        ...
        pygame.draw.circle(WIN, P_DOT_CLR[i], (x,y), dot.width/2)
        i += 1
    i = 0
    for dot in dots_n.values():
        x = GRID_RAD + np.sin(((dot.x+dot.width/2)-GRID_RAD)*(SEG_ANGLE/GRID_RAD))*(GRID_RAD/np.sin(SEG_ANGLE))
        y = GRID_RAD - np.sin((GRID_RAD-(dot.y+dot.height/2))*(SEG_ANGLE/GRID_RAD))*(GRID_RAD/np.sin(SEG_ANGLE))
        pygame.draw.circle(WIN, N_DOT_CLR[i], (x,y), dot.width/2)
        i += 1
...
def handle_collision_p(agent,dots_p,keys_pressed,score):
    for dot in dots_p.values():
        ...
        #(SPHERE_RAD*np.sin(SEG_ANGLE)+ dot.width)**2:
            dot.height = 0
            dot.width = 0
        else:
            dot.height = DOT_SZ
...
def handle_collision_n(agent,dots_n,keys_pressed,score):
    for dot in dots_n.values():
        ...
        x = GRID_RAD + (GRID_RAD/np.sin(SEG_ANGLE))*np.sin(((dot.x+dot.width/2)-GRID_RAD)*(SEG_ANGLE/GRID_RAD))
        y = GRID_RAD - (GRID_RAD/np.sin(SEG_ANGLE))*np.sin((GRID_RAD-(dot.y+dot.height/2))*(SEG_ANGLE/GRID_RAD))
        if ((x-GRID_RAD)**2 + (y-GRID_RAD)**2) >= (GRID_RAD-dot.width)**2:
        #(SPHERE_RAD*np.sin(SEG_ANGLE)+dot.width)**2:
            dot.height = 0
            dot.width = 0
        else:
